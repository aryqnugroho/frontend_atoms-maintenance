...
const getSignerStatus = (
  record: TfpAobLt12RecordSummary,
  ...
  if (
    (userRole === 'Supervisor TFP' || userRole === 'Admin') &&
...
export const TfpAobLt12ListPage: React.FC = () => {
  ...
  const [records, setRecords] = useState<TfpAobLt12RecordSummary[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await tfpAobLt12Service.listRecords(params);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        setErrorMessage('Gagal memuat daftar Performance Check AOB Lantai 1 & 2.');
      }
      setRecords([]);
      setTotalCount(0);
    } finally {
  ...
  const handleCreate = async () => {
    ...
    setErrorMessage(null);
    try {
      const record: TfpAobLt12RecordDetail = await tfpAobLt12Service.createRecord({
        date: getCurrentShiftDate(),
        shift_type: getCurrentShiftType(),
      });
      navigate(`/tfp/aob-lt12/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        ...
        if (err.response.status === 409 && data.errors?.existing_record) {
          navigate(`/tfp/aob-lt12/${data.errors.existing_record.id}`);
          return;
        }
        if (err.response.status === 422) {
          setErrorMessage(
            data.message ??
              'Tidak ada teknisi TFP yang bertugas pada shift ini. Pastikan roster sudah dipublish.',
          );
          return;
        }
        setErrorMessage(data.message ?? 'Gagal membuat Performance Check AOB Lantai 1 & 2.');
        return;
      }
      setErrorMessage('Gagal membuat Performance Check AOB Lantai 1 & 2. Coba lagi.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await tfpAobLt12Service.deleteRecord(id);
      void fetchRecords();
    } catch {
  ...
  if (debouncedSearch)
    activeFilters.push({
      ...
      clear: () => { setSearchQuery(''); setDebouncedSearch(''); },
    });
  if (dateFilter)
    activeFilters.push({ key: 'date', label: `Tgl: ${dateFilter}`, clear: () => setDateFilter('') });
  if (yearFilter)
    activeFilters.push({ key: 'year', label: `Tahun: ${yearFilter}`, clear: () => setYearFilter('') });
  if (shiftFilter)
    activeFilters.push({ key: 'shift', label: SHIFT_LABELS[shiftFilter] ?? shiftFilter, clear: () => setShiftFilter('') });
  if (statusFilter)
    activeFilters.push({ key: 'status', label: STATUS_LABELS[statusFilter] ?? statusFilter, clear: () => setStatusFilter('') });
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Performance Check AOB Lantai 1 & 2</span>
      </div>
      ...
      <PageHeader
        ...
        title="Performance Check AOB Lantai 1 & 2"
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 space-y-3">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              aria-label="Cari Performance Check AOB Lantai 1 & 2"
            />
          </div>
          {hasActiveFilters && (
            <button
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1">
            ...
            <>
              ...
              {activeFilters.map((f) => (
                <span key={f.key} className="inline-flex items-center gap-1 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-xs font-medium px-2.5 py-0.5">
                  {f.label}
                  <button onClick={f.clear} className="ml-0.5 rounded-full hover:bg-blue-100 transition-colors" aria-label={`Hapus filter ${f.label}`}>
                    <X size={11} />
                  </button>
                </span>
              ))}
          ...
          {!hasActiveFilters && !isLoading && (
            <span className="text-xs text-slate-400 ml-auto">{totalCount > 0 ? `${totalCount} Form` : ''}</span>
          )}
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            <div className="h-14 w-14 rounded-full bg-emerald-50 flex items-center justify-center">
              <TfpIcon size={24} className="text-maintenance-tfp/70" />
            </div>
            <p className="text-base font-semibold text-slate-700">Belum ada Performance Check AOB Lantai 1 & 2</p>
            <p className="text-sm text-slate-400 max-w-md">
              Klik <strong>Buat Performance Check</strong> untuk membuat form baru untuk shift hari ini.
            </p>
            {canCreate && (
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr
                      key={r.id}
                      onClick={() => navigate(`/tfp/aob-lt12/${r.id}`)}
                      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/tfp/aob-lt12/${r.id}`); } }}
                      tabIndex={0}
                      role="button"
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-slate-700">
                        {r.supervisor_name ? (
                          <Badge variant="tfp" className="text-xs">{r.supervisor_name}</Badge>
                        ) : (
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => navigate(`/tfp/aob-lt12/${r.id}`)} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Detail / Edit"><Edit2 size={16} /></button>
                          <button onClick={() => navigate(`/tfp/aob-lt12/${r.id}/print`)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Print"><Printer size={16} /></button>
                          {canDelete && (
                            <button onClick={() => handleDelete(r.id)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Hapus"><Trash2 size={16} /></button>
                          )}
                        </div>
                      </td>
                    </tr>
