...
import type { TfpTowerRecordDetail, TfpTowerItem } from '@/types/tfpTower';
...
const isModeRow = (item: TfpTowerItem): boolean =>
  item.parameter_name.toLowerCase().startsWith('mode');

const isSuplaiRow = (item: TfpTowerItem): boolean =>
  item.parameter_name.toLowerCase().startsWith('suplai aktif');

const isKwhRow = (item: TfpTowerItem): boolean =>
  item.parameter_name.toLowerCase().startsWith('kwh');
...
export const TfpTowerDetailPage: React.FC = () => {
  ...
  const fetchRecord = useCallback(async () => {
    ...
    try {
      ...
      data.items.forEach((item) => {
        iv[item.id] = {
          panel_a10: item.panel_a10 ?? '', panel_a11: item.panel_a11 ?? '',
          panel_ats_a13_input: item.panel_ats_a13_input ?? '', panel_ats_a13_output: item.panel_ats_a13_output ?? '',
          panel_a14: item.panel_a14 ?? '', panel_a16: item.panel_a16 ?? '',
          panel_a17: item.panel_a17 ?? '', panel_a18: item.panel_a18 ?? '',
          panel_a19: item.panel_a19 ?? '', panel_a20: item.panel_a20 ?? '',
          panel_milat_ru1213: item.panel_milat_ru1213 ?? '',
        };
      });
      setItemValues(iv);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await tfpTowerService.updateRecord(record.id, { items: itemsPayload, facilities: facilitiesPayload });
      setRecord(updated);
  ...
  if (!record) return (
    <div className="max-w-7xl mx-auto py-20 text-center">
      ...
      <Button onClick={() => navigate('/tfp/tower')} className="mt-4">Kembali ke Daftar</Button>
    </div>
  );
  ...
  const suplaiSelectCls = (val: string) => cn('w-full h-7 px-1.5 text-xs rounded border focus:ring-1 focus:outline-none font-medium',
    val === 'PLN 1' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : val === 'PLN 2' ? 'bg-sky-50 text-sky-700 border-sky-200' : 'bg-white border-slate-200 text-slate-700');
  ...
  return (
    <div className="max-w-full space-y-6 animate-fade-in pb-20">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <button type="button" onClick={() => navigate('/tfp/tower')} className="inline-flex items-center gap-1 hover:text-slate-700 transition-colors">Performance Check Gedung Tower</button>
        <span>/</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <Button variant="ghost" size="sm" onClick={() => navigate('/tfp/tower')} className="hover:bg-slate-100 mt-0.5"><ArrowLeft size={20} /></Button>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">Performance Check Gedung Tower</h1>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/tfp/tower/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50"><Printer size={15} />Print</Button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-6 items-start">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse" style={{ tableLayout: 'fixed', minWidth: '950px' }}>
              <colgroup>
              ...
              <thead>
                <tr className="bg-sky-800 text-white">
                  ...
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[9px]">Panel A 10 Tower Lt 11</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[9px]">Panel A 11 Tower Lt 11</th>
                  <th colSpan={2} className="px-1 py-1 text-center font-semibold border border-sky-700 text-[9px]">Panel ATS (A 13)</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[9px]">Panel A 14 Tower Lt 11</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[9px]">Panel A 16 ADC Room</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[9px]">Panel A 17 Ruang Lift</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[9px]">Panel A 18 Ruang RX</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[9px]">Panel A 19 Roof Top Power</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[9px]">Panel A 20 Power CCTV</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[9px]">Panel MILAT RU 12 &amp; 13</th>
                </tr>
              ...
              <tbody>
                {record.items.map((item, idx) => {
                  ...
                  if (isModeRow(item)) {
                    const atsVal = itemValues[item.id]?.panel_ats_a13_input ?? '';
                    return (
                      <tr key={item.id} className="bg-blue-50/40">
                        ...
                        <td colSpan={2} className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{atsVal || '—'}</span> : <select value={atsVal} onChange={(e) => setItemCell(item.id, 'panel_ats_a13_input', e.target.value)} className={modeSelectCls(atsVal)}><option value="">—</option><option value="Auto">Auto</option><option value="Manual">Manual</option></select>}
                        </td>
                        {['panel_a14','panel_a16','panel_a17','panel_a18','panel_a19','panel_a20','panel_milat_ru1213'].map((k) => (
                  ...
                  if (isSuplaiRow(item)) {
                    ...
                    return (
                      <tr key={item.id} className="bg-blue-50/40">
                        ...
                        <td className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                        <td colSpan={2} className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{atsVal || '—'}</span> : <select value={atsVal} onChange={(e) => setItemCell(item.id, 'panel_ats_a13_input', e.target.value)} className={suplaiSelectCls(atsVal)}><option value="">—</option><option value="PLN 1">PLN 1</option><option value="PLN 2">PLN 2</option></select>}
                        </td>
                        {['panel_a14','panel_a16','panel_a17','panel_a18','panel_a19','panel_a20','panel_milat_ru1213'].map((k) => (
                          <td key={k} className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                        ))}
                  ...
                  if (isKwhRow(item)) {
                    const val = itemValues[item.id]?.panel_ats_a13_input ?? '';
                    return (
                      <tr key={item.id} className={rowBase}>
                        ...
                        <td className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{val || '—'}</span> : <input type="text" inputMode="decimal" value={val} onChange={(e) => setItemCell(item.id, 'panel_ats_a13_input', e.target.value)} className="w-full h-7 px-1.5 text-center text-xs rounded border border-slate-200 bg-white focus:ring-1 focus:ring-sky-400 focus:outline-none" />}
                        </td>
                        {['panel_ats_a13_output','panel_a14','panel_a16','panel_a17','panel_a18','panel_a19','panel_a20','panel_milat_ru1213'].map((k) => (
                          <td key={k} className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                        ))}
                      </tr>
                    );
                  ...
                  if (isSuhuRow(item)) {
                    const val = itemValues[item.id]?.panel_a10 ?? '';
                    return (
                      <tr key={item.id} className={rowBase}>
                        <td className="px-1 py-1.5 text-slate-400 font-mono text-center text-xs border-r border-slate-100">{idx + 1}</td>
                        ...
                        <td className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{val || '—'}</span> : <input type="text" inputMode="decimal" value={val} onChange={(e) => setItemCell(item.id, 'panel_a10', e.target.value)} className="w-full h-7 px-1.5 text-center text-xs rounded border border-slate-200 bg-white focus:ring-1 focus:ring-sky-400 focus:outline-none" />}
                        </td>
                        {['panel_a11','panel_ats_a13_input','panel_ats_a13_output','panel_a14','panel_a16','panel_a17','panel_a18','panel_a19','panel_a20','panel_milat_ru1213'].map((k) => (
                          <td key={k} className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                        ))}
                      </tr>
                    );
      ...
      <TfpTowerSignaturePanel record={record} onUpdated={(updated) => {
        setRecord(updated);
        const iv: Record<number, Record<ItemColKey, string>> = {};
        updated.items.forEach((item) => {
          iv[item.id] = { panel_a10: item.panel_a10 ?? '', panel_a11: item.panel_a11 ?? '', panel_ats_a13_input: item.panel_ats_a13_input ?? '', panel_ats_a13_output: item.panel_ats_a13_output ?? '', panel_a14: item.panel_a14 ?? '', panel_a16: item.panel_a16 ?? '', panel_a17: item.panel_a17 ?? '', panel_a18: item.panel_a18 ?? '', panel_a19: item.panel_a19 ?? '', panel_a20: item.panel_a20 ?? '', panel_milat_ru1213: item.panel_milat_ru1213 ?? '' };
        });
        setItemValues(iv);
      }} />
    </div>
