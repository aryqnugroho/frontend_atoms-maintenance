...
import type { TfpGlidepathRecordDetail, TfpGlidepathRecordSummary } from '@/types/tfpGlidepath';
...
const getSignerStatus = (record: TfpGlidepathRecordSummary, userName?: string, userRole?: string): SignerBadgeStatus => {
...
export const TfpGlidepathListPage: React.FC = () => {
  ...
  const [records, setRecords] = useState<TfpGlidepathRecordSummary[]>([]);
  ...
  useEffect(() => {
    tfpGlidepathService.getYears().then(setAvailableYears).catch(() => setAvailableYears([new Date().getFullYear()]));
  }, []);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await tfpGlidepathService.listRecords(params);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        setErrorMessage('Gagal memuat daftar Performance Check Fasilitas Penunjang Gedung Glide Path.');
      }
  ...
  const handleCreate = async () => {
    ...
    try {
      const record: TfpGlidepathRecordDetail = await tfpGlidepathService.createRecord({ date: getCurrentShiftDate(), shift_type: getCurrentShiftType() });
      navigate(`/tfp/glidepath/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        const data = err.response.data as { message?: string; errors?: { existing_record?: { id: number } } };
        if (err.response.status === 409 && data.errors?.existing_record) { navigate(`/tfp/glidepath/${data.errors.existing_record.id}`); return; }
        if (err.response.status === 422) { setErrorMessage(data.message ?? 'Tidak ada teknisi TFP yang bertugas pada shift ini.'); return; }
        setErrorMessage(data.message ?? 'Gagal membuat Performance Check Fasilitas Penunjang Gedung Glide Path.');
        return;
      }
      setErrorMessage('Gagal membuat Performance Check Fasilitas Penunjang Gedung Glide Path. Coba lagi.');
    } finally { setIsCreating(false); }
  };

  const handleDelete = async (id: number) => {
    if (!confirm('Hapus form ini? Tindakan ini tidak dapat diurungkan.')) return;
    try { await tfpGlidepathService.deleteRecord(id); void fetchRecords(); }
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Performance Check Fasilitas Penunjang Gedung Glide Path</span>
      </div>

      <PageHeader icon={TfpIcon} iconBg="bg-emerald-100" iconColor="text-maintenance-tfp"
        title="Performance Check Fasilitas Penunjang Gedung Glide Path"
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Cari nomor form, manager, atau supervisor..." value={searchQuery}
              ...
              aria-label="Cari Performance Check Fasilitas Penunjang Gedung Glide Path" />
          </div>
          {hasActiveFilters && (
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            ...
            <p className="text-base font-semibold text-slate-700">Belum ada Performance Check Fasilitas Penunjang Gedung Glide Path</p>
            ...
            <table className="w-full text-sm min-w-[900px]">
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr key={r.id} onClick={() => navigate(`/tfp/glidepath/${r.id}`)}
                      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/tfp/glidepath/${r.id}`); } }}
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => navigate(`/tfp/glidepath/${r.id}`)} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Detail / Edit"><Edit2 size={16} /></button>
                          <button onClick={() => navigate(`/tfp/glidepath/${r.id}/print`)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Print"><Printer size={16} /></button>
