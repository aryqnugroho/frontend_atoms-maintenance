...
import type { TfpDvorRecordDetail, TfpDvorItem, TfpDvorFacility } from '@/types/tfpDvor';
...
const renderPrintCell = (item: TfpDvorItem, colKey: ItemColKey): React.ReactElement => {
  if (isDisabled(item, colKey)) return <td className="border border-black bg-gray-300 px-1 py-1" />;

  if (isModeRow(item)) {
    if (colKey === 'panel_d05') return <td className="border border-black px-1 py-1 text-center text-[10px] font-semibold">{val(item.panel_d05) || 'Auto / Manual'}</td>;
    if (colKey === 'panel_ats_d06_input') return <td className="border border-black px-1 py-1 text-center text-[10px] font-semibold">{val(item.panel_ats_d06_input) || 'Auto / Manual'}</td>;
    if (colKey === 'panel_ats_d06_output') return <td className="border border-black bg-gray-300 px-1 py-1" />;
    return <td className="border border-black px-1 py-1 text-center text-[10px]">{val(item[colKey] as string | null)}</td>;
  }

  if (isSuplaiRow(item)) {
    if (colKey === 'panel_d05') return <td className="border border-black px-1 py-1 text-center text-[10px] font-semibold">{val(item.panel_d05) || 'PLN / Genset'}</td>;
    if (colKey === 'panel_ats_d06_input') return <td className="border border-black px-1 py-1 text-center text-[10px] font-semibold">{val(item.panel_ats_d06_input) || 'PLN / Genset'}</td>;
    if (colKey === 'panel_ats_d06_output') return <td className="border border-black bg-gray-300 px-1 py-1" />;
    return <td className="border border-black px-1 py-1 text-center text-[10px]">{val(item[colKey] as string | null)}</td>;
  }

  if (isSingleValueRow(item)) {
    if (colKey === 'panel_d05') return <td className="border border-black px-1 py-1 text-center text-[10px]">{val(item.panel_d05)}</td>;
...
export const TfpDvorPrintView: React.FC = () => {
  ...
  if (!record) return (
    <div className="flex h-screen flex-col items-center justify-center gap-4">
      ...
      <Button variant="outline" onClick={() => navigate('/tfp/dvor')}>Kembali</Button>
    </div>
  );
  ...
  const padded: { item: TfpDvorItem | null; facility: TfpDvorFacility | null }[] = [];
  ...
  return (
    <div className="min-h-screen w-full bg-slate-100 p-4 text-black print:bg-white print:p-0">
      ...
      <div className="print-hide mx-auto mb-4 flex max-w-[290mm] items-center justify-between">
        <Button variant="outline" className="gap-2" onClick={() => navigate(`/tfp/dvor/${record.id}`)}>
      ...
      <div className="mx-auto max-w-[290mm] bg-white font-sans text-[10px] print:mx-0 print:w-full print:max-w-none">
        ...
        <div className="grid grid-cols-[40%_30%_30%] items-center mb-2 px-2 pt-3">
          ...
          <div className="text-center">
            <div className="text-[13px] font-black leading-tight">Performance Check Gedung DVOR</div>
          </div>
        ...
        <table className="w-full border-collapse text-[9px] mx-auto" style={{ tableLayout: 'fixed' }}>
          <colgroup>
            <col style={{ width: '20px' }} /><col style={{ width: '110px' }} />
            <col style={{ width: '40px' }} /><col style={{ width: '55px' }} />
            <col style={{ width: '55px' }} /><col style={{ width: '45px' }} />
            <col style={{ width: '38px' }} /><col style={{ width: '38px' }} />
            <col style={{ width: '120px' }} /><col style={{ width: '45px' }} /><col style={{ width: '80px' }} />
          </colgroup>
          <thead>
            <tr className="bg-blue-100">
              ...
              <th rowSpan={2} className="border border-black px-0.5 py-0.5 text-center font-bold text-[8px]">Panel D.01</th>
              <th rowSpan={2} className="border border-black px-0.5 py-0.5 text-center font-bold text-[8px]">Panel D.03 CCTV Indoor</th>
              <th rowSpan={2} className="border border-black px-0.5 py-0.5 text-center font-bold text-[8px]">Panel D.04 CCTV Outdoor</th>
              <th rowSpan={2} className="border border-black px-0.5 py-0.5 text-center font-bold text-[8px]">Panel Input D.05</th>
              <th colSpan={2} className="border border-black px-0.5 py-0.5 text-center font-bold text-[8px]">Panel ATS/AMF (D.06)</th>
              ...
              return (
                <tr key={`row-${idx}`}>
                  ...
                  {item
                    ...
                    : Array.from({ length: 6 }).map((_, i) => <td key={`pad-${i}`} className="border border-black px-1 py-1" />)}
