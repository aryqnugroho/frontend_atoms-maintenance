...
import type { TfpDvorRecordDetail, TfpDvorItem } from '@/types/tfpDvor';

type ItemColKey = 'panel_d01' | 'panel_d03' | 'panel_d04' | 'panel_d05' | 'panel_ats_d06_input' | 'panel_ats_d06_output';

const ALL_COL_KEYS: ItemColKey[] = ['panel_d01', 'panel_d03', 'panel_d04', 'panel_d05', 'panel_ats_d06_input', 'panel_ats_d06_output'];

const isDisabled = (item: TfpDvorItem, colKey: ItemColKey): boolean => item.is_disabled_map?.[colKey] === true;
const isModeRow = (item: TfpDvorItem): boolean => item.parameter_name.toLowerCase().startsWith('mode');
const isSuplaiRow = (item: TfpDvorItem): boolean => item.parameter_name.toLowerCase().startsWith('suplai aktif');
const isSingleValueRow = (item: TfpDvorItem): boolean => {
  const n = item.parameter_name.toLowerCase();
  return n.startsWith('suhu ruangan') || n.startsWith('kwh');
};

interface CellInputProps {
  item: TfpDvorItem; colKey: ItemColKey; value: string;
...
export const TfpDvorDetailPage: React.FC = () => {
  ...
  const fetchRecord = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const data = await tfpDvorService.getRecord(Number(id));
      setRecord(data);
      const iv: Record<number, Record<ItemColKey, string>> = {};
      data.items.forEach((item) => {
        iv[item.id] = {
          panel_d01: item.panel_d01 ?? '', panel_d03: item.panel_d03 ?? '',
          panel_d04: item.panel_d04 ?? '', panel_d05: item.panel_d05 ?? '',
          panel_ats_d06_input: item.panel_ats_d06_input ?? '', panel_ats_d06_output: item.panel_ats_d06_output ?? '',
        };
      });
      setItemValues(iv);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await tfpDvorService.updateRecord(record.id, { items: itemsPayload, facilities: facilitiesPayload });
      setRecord(updated);
  ...
  if (!record) return (
    <div className="max-w-7xl mx-auto py-20 text-center">
      ...
      <Button onClick={() => navigate('/tfp/dvor')} className="mt-4">Kembali ke Daftar</Button>
    </div>
  );
  ...
  const suplaiSelectCls = (val: string) => cn('w-full h-7 px-1.5 text-xs rounded border focus:ring-1 focus:outline-none font-medium',
    val === 'PLN' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : val === 'Genset' ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-white border-slate-200 text-slate-700');
  ...
  return (
    <div className="max-w-full space-y-6 animate-fade-in pb-20">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <button type="button" onClick={() => navigate('/tfp/dvor')} className="inline-flex items-center gap-1 hover:text-slate-700 transition-colors">Performance Check Gedung DVOR</button>
        <span>/</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <Button variant="ghost" size="sm" onClick={() => navigate('/tfp/dvor')} className="hover:bg-slate-100 mt-0.5"><ArrowLeft size={20} /></Button>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">Performance Check Gedung DVOR</h1>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/tfp/dvor/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50"><Printer size={15} />Print</Button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-6 items-start">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse" style={{ tableLayout: 'fixed', minWidth: '600px' }}>
              <colgroup>
                <col style={{ width: '28px' }} /><col style={{ width: '160px' }} />
              ...
              <thead>
                <tr className="bg-sky-800 text-white">
                  ...
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[10px]">Panel D.01</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[10px]">Panel D.03 CCTV Indoor</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[10px]">Panel D.04 CCTV Outdoor</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[10px]">Panel Input D.05</th>
                  <th colSpan={2} className="px-1 py-1 text-center font-semibold border border-sky-700 text-[10px]">Panel ATS/AMF (D.06)</th>
                </tr>
              ...
              <tbody>
                {record.items.map((item, idx) => {
                  ...
                  if (isModeRow(item)) {
                    const d05Val = itemValues[item.id]?.panel_d05 ?? '';
                    const atsInVal = itemValues[item.id]?.panel_ats_d06_input ?? '';
                    return (
                      <tr key={item.id} className="bg-blue-50/40">
                        ...
                        <td className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{d05Val || '—'}</span> : <select value={d05Val} onChange={(e) => setItemCell(item.id, 'panel_d05', e.target.value)} className={modeSelectCls(d05Val)}><option value="">—</option><option value="Auto">Auto</option><option value="Manual">Manual</option></select>}
                        </td>
                        <td colSpan={2} className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{atsInVal || '—'}</span> : <select value={atsInVal} onChange={(e) => setItemCell(item.id, 'panel_ats_d06_input', e.target.value)} className={modeSelectCls(atsInVal)}><option value="">—</option><option value="Auto">Auto</option><option value="Manual">Manual</option></select>}
                        </td>
                      </tr>
                    );
                  }
                  ...
                  if (isSuplaiRow(item)) {
                    ...
                    return (
                      <tr key={item.id} className="bg-blue-50/40">
                        ...
                        <td className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                        <td className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{d05Val || '—'}</span> : <select value={d05Val} onChange={(e) => setItemCell(item.id, 'panel_d05', e.target.value)} className={suplaiSelectCls(d05Val)}><option value="">—</option><option value="PLN">PLN</option><option value="Genset">Genset</option></select>}
                        </td>
                        <td colSpan={2} className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{atsInVal || '—'}</span> : <select value={atsInVal} onChange={(e) => setItemCell(item.id, 'panel_ats_d06_input', e.target.value)} className={suplaiSelectCls(atsInVal)}><option value="">—</option><option value="PLN">PLN</option><option value="Genset">Genset</option></select>}
                        </td>
                      </tr>
                    );
                  }
                  ...
                  if (isSingleValueRow(item)) {
                    const val = itemValues[item.id]?.panel_d05 ?? '';
                    return (
                      <tr key={item.id} className={rowBase}>
                        ...
                        <td className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                        <td className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{val || '—'}</span> : <input type="text" inputMode="decimal" value={val} onChange={(e) => setItemCell(item.id, 'panel_d05', e.target.value)} className="w-full h-7 px-1.5 text-center text-xs rounded border border-slate-200 bg-white focus:ring-1 focus:ring-sky-400 focus:outline-none" />}
                        </td>
                        <td className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                        <td className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                      </tr>
      ...
      <TfpDvorSignaturePanel record={record} onUpdated={(updated) => {
        setRecord(updated);
        const iv: Record<number, Record<ItemColKey, string>> = {};
        updated.items.forEach((item) => {
          iv[item.id] = { panel_d01: item.panel_d01 ?? '', panel_d03: item.panel_d03 ?? '', panel_d04: item.panel_d04 ?? '', panel_d05: item.panel_d05 ?? '', panel_ats_d06_input: item.panel_ats_d06_input ?? '', panel_ats_d06_output: item.panel_ats_d06_output ?? '' };
        });
        setItemValues(iv);
      }} />
    </div>
