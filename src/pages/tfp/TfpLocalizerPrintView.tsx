...
import type { TfpLocalizerRecordDetail, TfpLocalizerItem, TfpLocalizerFacility } from '@/types/tfpLocalizer';

type ItemColKey = 'panel_lz01' | 'panel_cos_lz02_input' | 'panel_cos_lz02_output' | 'panel_lz03' | 'panel_cos_lz04_input' | 'panel_cos_lz04_output' | 'panel_mlat_ru04';
const ALL_COL_KEYS: ItemColKey[] = ['panel_lz01', 'panel_cos_lz02_input', 'panel_cos_lz02_output', 'panel_lz03', 'panel_cos_lz04_input', 'panel_cos_lz04_output', 'panel_mlat_ru04'];
...
const renderPrintCell = (item: TfpLocalizerItem, colKey: ItemColKey): React.ReactElement => {
  if (isDisabled(item, colKey)) return <td className="border border-black bg-gray-300 px-1 py-1" />;

  if (isModeRow(item)) {
    if (colKey === 'panel_cos_lz02_input') return <td className="border border-black px-1 py-1 text-center text-[10px] font-semibold">{val(item.panel_cos_lz02_input) || 'Auto / Manual'}</td>;
    if (colKey === 'panel_cos_lz02_output') return <td className="border border-black bg-gray-300 px-1 py-1" />;
    if (colKey === 'panel_cos_lz04_input') return <td className="border border-black px-1 py-1 text-center text-[10px] font-semibold">{val(item.panel_cos_lz04_input) || 'Auto / Manual'}</td>;
    if (colKey === 'panel_cos_lz04_output') return <td className="border border-black bg-gray-300 px-1 py-1" />;
    return <td className="border border-black px-1 py-1 text-center text-[10px]">{val(item[colKey] as string | null)}</td>;
  }

  if (isSuplaiRow(item)) {
    if (colKey === 'panel_cos_lz02_input') return <td className="border border-black px-1 py-1 text-center text-[10px] font-semibold">{val(item.panel_cos_lz02_input) || 'PLN / UPS'}</td>;
    if (colKey === 'panel_cos_lz02_output') return <td className="border border-black bg-gray-300 px-1 py-1" />;
    if (colKey === 'panel_cos_lz04_input') return <td className="border border-black px-1 py-1 text-center text-[10px] font-semibold">{val(item.panel_cos_lz04_input) || 'PLN / UPS'}</td>;
    if (colKey === 'panel_cos_lz04_output') return <td className="border border-black bg-gray-300 px-1 py-1" />;
    return <td className="border border-black px-1 py-1 text-center text-[10px]">{val(item[colKey] as string | null)}</td>;
  }

  if (isSingleValueRow(item)) {
    if (colKey === 'panel_lz01') return <td className="border border-black px-1 py-1 text-center text-[10px]">{val(item.panel_lz01)}</td>;
...
export const TfpLocalizerPrintView: React.FC = () => {
  ...
  if (!record) return (
    <div className="flex h-screen flex-col items-center justify-center gap-4">
      ...
      <Button variant="outline" onClick={() => navigate('/tfp/localizer')}>Kembali</Button>
    </div>
  );
  ...
  const padded: { item: TfpLocalizerItem | null; facility: TfpLocalizerFacility | null }[] = [];
  ...
  return (
    <div className="min-h-screen w-full bg-slate-100 p-4 text-black print:bg-white print:p-0">
      ...
      <div className="print-hide mx-auto mb-4 flex max-w-[290mm] items-center justify-between">
        <Button variant="outline" className="gap-2" onClick={() => navigate(`/tfp/localizer/${record.id}`)}>
      ...
      <div className="mx-auto max-w-[290mm] bg-white font-sans text-[10px] print:mx-0 print:w-full print:max-w-none">
        ...
        <div className="grid grid-cols-[40%_30%_30%] items-center mb-2 px-2 pt-3">
          ...
          <div className="text-center">
            <div className="text-[13px] font-black leading-tight">Performance Check Gedung Localizer</div>
          </div>
        ...
        <table className="w-full border-collapse text-[9px] mx-auto" style={{ tableLayout: 'fixed' }}>
          <colgroup>
            ...
            <col style={{ width: '35px' }} /><col style={{ width: '35px' }} />
            <col style={{ width: '38px' }} />
            <col style={{ width: '35px' }} /><col style={{ width: '35px' }} />
            <col style={{ width: '40px' }} />
            <col style={{ width: '115px' }} /><col style={{ width: '45px' }} /><col style={{ width: '80px' }} />
          </colgroup>
          <thead>
            <tr className="bg-blue-100">
              ...
              <th rowSpan={2} className="border border-black px-0.5 py-0.5 text-center font-bold text-[8px]">Panel LZ 01</th>
              <th colSpan={2} className="border border-black px-0.5 py-0.5 text-center font-bold text-[8px]">Panel COS (LZ 02)</th>
              <th rowSpan={2} className="border border-black px-0.5 py-0.5 text-center font-bold text-[8px]">Panel LZ 03</th>
              <th colSpan={2} className="border border-black px-0.5 py-0.5 text-center font-bold text-[8px]">Panel COS (LZ 04)</th>
              <th rowSpan={2} className="border border-black px-0.5 py-0.5 text-center font-bold text-[8px]">Panel MLAT RU 04</th>
              ...
              return (
                <tr key={`row-${idx}`}>
                  ...
                  {item
                    ...
                    : Array.from({ length: 7 }).map((_, i) => <td key={`pad-${i}`} className="border border-black px-1 py-1" />)}
