...
import type { TfpGlidepathRecordDetail, TfpGlidepathItem, TfpGlidepathFacility } from '@/types/tfpGlidepath';
...
export const TfpGlidepathPrintView: React.FC = () => {
  ...
  const padded: { item: TfpGlidepathItem | null; facility: TfpGlidepathFacility | null }[] = [];
      ...
      <div className="mx-auto max-w-[290mm] bg-white font-sans text-[10px] print:mx-0 print:w-full print:max-w-none">
        ...
        <div className="grid grid-cols-[40%_30%_30%] items-center mb-2 px-2 pt-3">
          ...
          <div className="text-center">
            <div className="text-[13px] font-black leading-tight">Performance Check Fasilitas Penunjang Gedung Glide Path</div>
          </div>
        ...
        <table className="w-full border-collapse text-[9px] mx-auto" style={{ tableLayout: 'fixed' }}>
          <colgroup>
            <col style={{ width: '20px' }} /><col style={{ width: '130px' }} />
            <col style={{ width: '120px' }} />
            <col style={{ width: '130px' }} /><col style={{ width: '50px' }} /><col style={{ width: '100px' }} />
          </colgroup>
          <thead>
            <tr className="bg-blue-100">
              <th className="border border-black px-1 py-1 text-center font-bold text-[9px]">No.</th>
              <th className="border border-black px-1 py-1 text-center font-bold text-[9px]">Parameter</th>
              <th className="border border-black px-1 py-1 text-center font-bold text-[9px]">Panel GP 01</th>
              <th className="border border-black px-1 py-1 text-center font-bold text-[9px]">Nama Fasilitas</th>
              <th className="border border-black px-1 py-1 text-center font-bold text-[9px]">Kondisi</th>
              <th className="border border-black px-1 py-1 text-center font-bold text-[9px]">Keterangan</th>
            </tr>
          </thead>
          <tbody>
            {padded.map((row, idx) => {
              ...
              const disabled = item?.is_disabled_map?.['panel_gp01'] === true;
              return (
                <tr key={`row-${idx}`}>
                  ...
                  <td className={`border border-black px-1 py-1 text-center text-[9px] ${disabled ? 'bg-gray-300' : ''}`}>
                    {item && !disabled ? val(item.panel_gp01) : ''}
                  </td>
