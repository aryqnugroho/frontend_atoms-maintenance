...
const flattenColumns = (config: TfpRadarColumnsConfig): FlatCell[] => {
  const out: FlatCell[] = [];
  let i = 0;
...
interface AddPanelModalProps {
  open: boolean;
  onClose: () => void;
  onAdd: (panel: TfpRadarPanel) => void;
  existingIds: string[];
}
...
export const TfpRadarDetailPage: React.FC = () => {
  ...
  const [draftConfig, setDraftConfig] = useState<TfpRadarColumnsConfig | null>(null);
  ...
  const hydrate = (data: TfpRadarRecordDetail) => {
    setRecord(data);
  ...
  const effectiveConfig: TfpRadarColumnsConfig = useMemo(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await tfpRadarService.updateRecord(record.id, {
        items: itemsPayload,
  ...
  const handleSaveStructure = async () => {
    ...
    try {
      ...
      const updated = await tfpRadarService.saveStructure(record.id, {
  ...
  const mutateConfig = (fn: (cfg: TfpRadarColumnsConfig) => TfpRadarColumnsConfig) => {
    setDraftConfig((prev) => {
  ...
  const handleAddPanel = (panel: TfpRadarPanel) => {
    mutateConfig((cfg) => [...cfg, panel]);
  };

  // ─── Existing structure ops ────────────────────────────────────────────

  const withStructureError = async (fn: () => Promise<TfpRadarRecordDetail>) => {
    setErrorMessage(null);
    try {
  ...
  const handleAddParameter = (data: { parameter_name: string; parameter_number?: string | null; unit?: string | null }) =>
    withStructureError(() => tfpRadarService.addParameter(record!.id, data));

  const handleUpdateParameter = (paramId: number, patch: { parameter_number?: string | null; parameter_name?: string; unit?: string | null }) =>
    withStructureError(() => tfpRadarService.updateParameter(record!.id, paramId, patch));
  ...
  const handleDeleteParameter = (paramId: number, name: string) => {
    ...
    void withStructureError(() => tfpRadarService.deleteParameter(record!.id, paramId));
  };
  ...
  const handleMoveParameter = (idx: number, dir: -1 | 1) => {
    ...
    void withStructureError(() => tfpRadarService.reorderParameters(record.id, ids));
  };

  const handleAddFacility = (name: string) =>
    withStructureError(() => tfpRadarService.addFacility(record!.id, { facility_name: name }));

  const handleUpdateFacility = (facilityId: number, name: string) =>
    withStructureError(() => tfpRadarService.updateFacility(record!.id, facilityId, { facility_name: name }));

  const handleDeleteFacility = (facilityId: number, name: string) => {
    if (!window.confirm(`Hapus fasilitas "${name}"?`)) return;
    void withStructureError(() => tfpRadarService.deleteFacility(record!.id, facilityId));
  };
  ...
  const handleMoveFacility = (idx: number, dir: -1 | 1) => {
    ...
    if (target < 0 || target >= ids.length) return;
    [ids[idx], ids[target]] = [ids[target], ids[idx]];
    void withStructureError(() => tfpRadarService.reorderFacilities(record.id, ids));
  };
  ...
  return (
    <div className="max-w-full space-y-6 animate-fade-in pb-20">
      ...
      <TfpRadarSignaturePanel record={record} onUpdated={hydrate} />
