...
import { TfpRadarService } from '@/services/tfpRadarService';
import { TfpRadarSignaturePanel } from './components/TfpRadarSignaturePanel';
import { cn } from '@/lib/utils';
import type { TfpRadarRecordDetail, TfpRadarItem } from '@/types/tfpRadar';

type ItemColKey =
  | 'panel_rd01' | 'panel_rd02'
  | 'panel_cos_rd03_input' | 'panel_cos_rd03_output'
  | 'ups_topaz_input' | 'ups_topaz_output' | 'panel_rd04' | 'panel_rd05' | 'panel_rd06' | 'panel_rd07'
  | 'panel_rd08';
...
const isDisabled = (item: TfpRadarItem, colKey: ItemColKey): boolean =>
  item.is_disabled_map?.[colKey] === true;

const isModeRow = (item: TfpRadarItem): boolean =>
  item.parameter_name.toLowerCase().startsWith('mode');

const isSuplaiRow = (item: TfpRadarItem): boolean =>
  item.parameter_name.toLowerCase().startsWith('suplai aktif');

const isKwhRow = (item: TfpRadarItem): boolean =>
...
export const TfpRadarDetailPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<TfpRadarRecordDetail | null>(null);
  ...
  const fetchRecord = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const data = await TfpRadarService.getRecord(Number(id));
      setRecord(data);
      const iv: Record<number, Record<ItemColKey, string>> = {};
      data.items.forEach((item) => {
        iv[item.id] = {
          panel_rd01: item.panel_rd01 ?? '', panel_rd02: item.panel_rd02 ?? '',
          panel_cos_rd03_input: item.panel_cos_rd03_input ?? '', panel_cos_rd03_output: item.panel_cos_rd03_output ?? '',
          ups_topaz_input: item.ups_topaz_input ?? '', ups_topaz_output: item.ups_topaz_output ?? '',
          panel_rd04: item.panel_rd04 ?? '', panel_rd05: item.panel_rd05 ?? '',
          panel_rd06: item.panel_rd06 ?? '', panel_rd07: item.panel_rd07 ?? '',
          panel_rd08: item.panel_rd08 ?? '',
        };
      });
      setItemValues(iv);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await TfpRadarService.updateRecord(record.id, { items: itemsPayload, facilities: facilitiesPayload });
      setRecord(updated);
  ...
  if (!record) return (
    <div className="max-w-7xl mx-auto py-20 text-center">
      ...
      <Button onClick={() => navigate('/tfp/radar-tfp')} className="mt-4">Kembali ke Daftar</Button>
    </div>
  );
  ...
  return (
    <div className="max-w-full space-y-6 animate-fade-in pb-20">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <Button variant="ghost" size="sm" onClick={() => navigate('/tfp/radar-tfp')} className="hover:bg-slate-100 mt-0.5"><ArrowLeft size={20} /></Button>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">Performance Check Gedung Radar</h1>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/tfp/radar-tfp/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50"><Printer size={15} />Print</Button>
          </div>
        </div>
      ...
      <TfpRadarSignaturePanel record={record} onUpdated={(updated) => {
        setRecord(updated);
        const iv: Record<number, Record<ItemColKey, string>> = {};
        updated.items.forEach((item) => {
          iv[item.id] = { panel_rd01: item.panel_rd01 ?? '', panel_rd02: item.panel_rd02 ?? '', panel_cos_rd03_input: item.panel_cos_rd03_input ?? '', panel_cos_rd03_output: item.panel_cos_rd03_output ?? '', ups_topaz_input: item.ups_topaz_input ?? '', ups_topaz_output: item.ups_topaz_output ?? '', panel_rd04: item.panel_rd04 ?? '', panel_rd05: item.panel_rd05 ?? '', panel_rd06: item.panel_rd06 ?? '', panel_rd07: item.panel_rd07 ?? '', panel_rd08: item.panel_rd08 ?? '' };
        });
        setItemValues(iv);
      }} />
    </div>
