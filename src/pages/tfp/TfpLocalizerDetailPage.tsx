...
import type {
  ...
  TfpLocalizerItem,
  TfpLocalizerFacility,
  TfpLocalizerColumnsConfig,
  TfpLocalizerPanel,
...
const flattenColumns = (config: TfpLocalizerColumnsConfig): FlatCell[] => {
  const out: FlatCell[] = [];
  let i = 0;
...
const isModeRow = (item: TfpLocalizerItem): boolean =>
  item.parameter_name.toLowerCase().startsWith('mode');

const isSuplaiRow = (item: TfpLocalizerItem): boolean =>
...
interface AddPanelModalProps {
  open: boolean;
  onClose: () => void;
  onAdd: (panel: TfpLocalizerPanel) => void;
  existingIds: string[];
}
...
export const TfpLocalizerDetailPage: React.FC = () => {
  ...
  const [draftConfig, setDraftConfig] = useState<TfpLocalizerColumnsConfig | null>(null);
  ...
  const effectiveConfig: TfpLocalizerColumnsConfig = useMemo(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await tfpLocalizerService.updateRecord(record.id, {
        items: itemsPayload,
  ...
  const handleSaveStructure = async () => {
    ...
    try {
      ...
      const updated = await tfpLocalizerService.saveStructure(record.id, {
  ...
  const mutateConfig = (fn: (cfg: TfpLocalizerColumnsConfig) => TfpLocalizerColumnsConfig) => {
    setDraftConfig((prev) => {
  ...
  const handleAddPanel = (panel: TfpLocalizerPanel) => {
    mutateConfig((cfg) => [...cfg, panel]);
  };

  // ─── Existing structure ops ────────────────────────────────────────────

  const withStructureError = async (fn: () => Promise<TfpLocalizerRecordDetail>) => {
    setErrorMessage(null);
    try {
  ...
  const handleAddParameter = (data: { parameter_name: string; parameter_number?: string | null; unit?: string | null }) =>
    withStructureError(() => tfpLocalizerService.addParameter(record!.id, data));

  const handleUpdateParameter = (paramId: number, patch: { parameter_number?: string | null; parameter_name?: string; unit?: string | null }) =>
    withStructureError(() => tfpLocalizerService.updateParameter(record!.id, paramId, patch));
  ...
  const handleDeleteParameter = (paramId: number, name: string) => {
    ...
    void withStructureError(() => tfpLocalizerService.deleteParameter(record!.id, paramId));
  };
  ...
  const handleMoveParameter = (idx: number, dir: -1 | 1) => {
    ...
    void withStructureError(() => tfpLocalizerService.reorderParameters(record.id, ids));
  };

  const handleAddFacility = (name: string) =>
    withStructureError(() => tfpLocalizerService.addFacility(record!.id, { facility_name: name }));

  const handleUpdateFacility = (facilityId: number, name: string) =>
    withStructureError(() => tfpLocalizerService.updateFacility(record!.id, facilityId, { facility_name: name }));

  const handleDeleteFacility = (facilityId: number, name: string) => {
    if (!window.confirm(`Hapus fasilitas "${name}"?`)) return;
    void withStructureError(() => tfpLocalizerService.deleteFacility(record!.id, facilityId));
  };
  ...
  const handleMoveFacility = (idx: number, dir: -1 | 1) => {
    ...
    if (target < 0 || target >= ids.length) return;
    [ids[idx], ids[target]] = [ids[target], ids[idx]];
    void withStructureError(() => tfpLocalizerService.reorderFacilities(record.id, ids));
  };
  ...
  if (!record) {
    return (
      <div className="max-w-7xl mx-auto py-20 text-center">
        ...
        <Button onClick={() => navigate('/tfp/localizer')} className="mt-4">
          Kembali ke Daftar
        </Button>
      </div>
    );
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <Button variant="ghost" size="sm" onClick={() => navigate('/tfp/localizer')} className="hover:bg-slate-100 mt-0.5">
              <ArrowLeft size={20} />
            </Button>
            <div>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm"
              onClick={() => navigate(`/tfp/localizer/${record.id}/print`)}
