...
import type { TfpTransmitterTxRecordDetail, TfpTransmitterTxItem, TfpTransmitterTxFacility } from '@/types/tfpTransmitterTx';
...
const renderPrintCell = (item: TfpTransmitterTxItem, colKey: ItemColKey): React.ReactElement => {
  if (isDisabled(item, colKey)) return <td className="border border-black bg-gray-300 px-1 py-1" />;

  if (isModeRow(item)) {
    if (colKey === 'panel_cos_tx03_input') {
      return <td className="border border-black px-1 py-1 text-center text-[10px] font-semibold">{val(item.panel_cos_tx03_input) || 'Auto / Manual'}</td>;
    }
    if (colKey === 'panel_ups_tx07_input') {
      return <td className="border border-black px-1 py-1 text-center text-[10px] font-semibold">{val(item.panel_ups_tx07_input) || 'Auto / Manual'}</td>;
    }
    if (colKey === 'panel_cos_tx03_output' || colKey === 'panel_ups_tx07_output') {
  ...
  if (isSuplaiRow(item)) {
    if (colKey === 'panel_cos_tx03_input') {
      return <td className="border border-black px-1 py-1 text-center text-[10px] font-semibold">{val(item.panel_cos_tx03_input) || 'PLN / UPS'}</td>;
    }
    if (colKey === 'panel_ups_tx07_input') {
      return <td className="border border-black px-1 py-1 text-center text-[10px] font-semibold">{val(item.panel_ups_tx07_input) || 'PLN / UPS'}</td>;
    }
    if (colKey === 'panel_cos_tx03_output' || colKey === 'panel_ups_tx07_output') {
      return <td className="border border-black bg-gray-300 px-1 py-1" />;
    }
  ...
  if (isSingleValueRow(item) && colKey !== 'panel_tx01') {
    return <td className="border border-black bg-gray-300 px-1 py-1" />;
  }
...
export const TfpTransmitterTxPrintView: React.FC = () => {
  ...
  if (!record) return (
    <div className="flex h-screen flex-col items-center justify-center gap-4">
      ...
      <Button variant="outline" onClick={() => navigate('/tfp/transmitter-tx')}>Kembali</Button>
    </div>
  );
  ...
  const padded: { item: TfpTransmitterTxItem | null; facility: TfpTransmitterTxFacility | null }[] = [];
  ...
  return (
    <div className="min-h-screen w-full bg-slate-100 p-4 text-black print:bg-white print:p-0">
      ...
      <div className="print-hide mx-auto mb-4 flex max-w-[290mm] items-center justify-between">
        <Button variant="outline" className="gap-2" onClick={() => navigate(`/tfp/transmitter-tx/${record.id}`)}>
      ...
      <div className="mx-auto max-w-[290mm] bg-white font-sans text-[10px] print:mx-0 print:w-full print:max-w-none">
        ...
        <div className="grid grid-cols-[40%_30%_30%] items-center mb-2 px-2 pt-3">
          ...
          <div className="text-center">
            <div className="text-[13px] font-black leading-tight">Performance Check Gedung (Transmitter) TX</div>
          </div>
        ...
        <table className="w-full border-collapse text-[9px] mx-auto" style={{ tableLayout: 'fixed' }}>
          <colgroup>
            <col style={{ width: '20px' }} /><col style={{ width: '100px' }} />
            ...
            <col style={{ width: '32px' }} /><col style={{ width: '32px' }} />
            <col style={{ width: '32px' }} />
            <col style={{ width: '110px' }} /><col style={{ width: '45px' }} /><col style={{ width: '80px' }} />
          </colgroup>
          <thead>
            <tr className="bg-blue-100">
              ...
              <th rowSpan={2} className="border border-black px-1 py-0.5 text-center font-bold text-[8px]">Panel TX 01</th>
              <th rowSpan={2} className="border border-black px-1 py-0.5 text-center font-bold text-[8px]">Panel TX 02</th>
              <th colSpan={2} className="border border-black px-1 py-0.5 text-center font-bold text-[8px]">Panel COS (TX 03)</th>
              <th rowSpan={2} className="border border-black px-1 py-0.5 text-center font-bold text-[8px]">Panel Output UPS TX 04</th>
              <th colSpan={2} className="border border-black px-1 py-0.5 text-center font-bold text-[8px]">Panel UPS (TX 07)</th>
              <th rowSpan={2} className="border border-black px-1 py-0.5 text-center font-bold text-[8px]">Panel AC TX 06</th>
              <th colSpan={2} className="border border-black px-1 py-0.5 text-center font-bold text-[8px]">UPS PILLER</th>
              <th rowSpan={2} className="border border-black px-1 py-0.5 text-center font-bold text-[8px]">Panel MILAT RU 11</th>
            ...
            <tr className="bg-blue-50 italic">
              {['Input','Output','Input','Output','Input','Output'].map((lbl, i) => (
                <th key={i} className="border border-black px-0.5 py-0.5 text-center font-semibold text-[8px]">{lbl}</th>
              ))}
            </tr>
          </thead>
          <tbody>
