...
export const TfpAobLt12PrintView: React.FC = () => {
  ...
  if (!record) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4">
        ...
        <Button variant="outline" onClick={() => navigate('/tfp/aob-lt12')}>Kembali</Button>
      </div>
    );
  }
  ...
  // Hide sub-column header row when every panel has exactly one sub-column.
  const hasMultiSubCols = config.some((p) => p.sub_columns.length > 1);

  const maxRows = Math.max(items.length, facilities.length);
  const padded: { item: TfpAobLt12Item | null; facility: typeof facilities[number] | null }[] = [];
  for (let i = 0; i < maxRows; i++) {
    padded.push({ item: items[i] ?? null, facility: facilities[i] ?? null });
  }

  const cellW = totalCellCount <= 8 ? 50 : totalCellCount <= 12 ? 38 : 30;
  ...
  return (
    <div className="min-h-screen w-full bg-slate-100 p-4 text-black print:bg-white print:p-0">
      ...
      <div className="print-hide mx-auto mb-4 flex max-w-[290mm] items-center justify-between">
        <Button variant="outline" className="gap-2" onClick={() => navigate(`/tfp/aob-lt12/${record.id}`)}>
          <ArrowLeft size={16} />
          Kembali
        </Button>
      ...
      <div className="mx-auto max-w-[290mm] bg-white font-sans text-[10px] print:mx-0 print:w-full print:max-w-none">
        ...
        <div className="grid grid-cols-[40%_30%_30%] items-center mb-2 px-2 pt-3">
          ...
          <div className="text-center">
            <div className="text-[13px] font-black leading-tight">Performance Check AOB Lantai 1 &amp; 2</div>
          </div>
        ...
        {/* Signature footer — compact grid (no table) so it stays on 1 page */}
