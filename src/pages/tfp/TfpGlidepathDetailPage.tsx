...
import type {
  ...
  TfpGlidepathItem,
  TfpGlidepathFacility,
  TfpGlidepathColumnsConfig,
  TfpGlidepathPanel,
...
const flattenColumns = (config: TfpGlidepathColumnsConfig): FlatCell[] => {
  const out: FlatCell[] = [];
  let i = 0;
...
interface AddPanelModalProps {
  open: boolean;
  onClose: () => void;
  onAdd: (panel: TfpGlidepathPanel) => void;
  existingIds: string[];
}
...
export const TfpGlidepathDetailPage: React.FC = () => {
  ...
  const [draftConfig, setDraftConfig] = useState<TfpGlidepathColumnsConfig | null>(null);
  ...
  const effectiveConfig: TfpGlidepathColumnsConfig = useMemo(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await tfpGlidepathService.updateRecord(record.id, {
        items: itemsPayload,
  ...
  const handleSaveStructure = async () => {
    ...
    try {
      ...
      const updated = await tfpGlidepathService.saveStructure(record.id, {
  ...
  const mutateConfig = (fn: (cfg: TfpGlidepathColumnsConfig) => TfpGlidepathColumnsConfig) => {
    setDraftConfig((prev) => {
  ...
  const handleAddPanel = (panel: TfpGlidepathPanel) => {
    mutateConfig((cfg) => [...cfg, panel]);
  };

  // ─── Existing structure ops ────────────────────────────────────────────

  const withStructureError = async (fn: () => Promise<TfpGlidepathRecordDetail>) => {
    setErrorMessage(null);
    try {
  ...
  const handleAddParameter = (data: { parameter_name: string; parameter_number?: string | null; unit?: string | null }) =>
    withStructureError(() => tfpGlidepathService.addParameter(record!.id, data));

  const handleUpdateParameter = (paramId: number, patch: { parameter_number?: string | null; parameter_name?: string; unit?: string | null }) =>
    withStructureError(() => tfpGlidepathService.updateParameter(record!.id, paramId, patch));
  ...
  const handleDeleteParameter = (paramId: number, name: string) => {
    ...
    void withStructureError(() => tfpGlidepathService.deleteParameter(record!.id, paramId));
  };
  ...
  const handleMoveParameter = (idx: number, dir: -1 | 1) => {
    ...
    void withStructureError(() => tfpGlidepathService.reorderParameters(record.id, ids));
  };

  const handleAddFacility = (name: string) =>
    withStructureError(() => tfpGlidepathService.addFacility(record!.id, { facility_name: name }));

  const handleUpdateFacility = (facilityId: number, name: string) =>
    withStructureError(() => tfpGlidepathService.updateFacility(record!.id, facilityId, { facility_name: name }));

  const handleDeleteFacility = (facilityId: number, name: string) => {
    if (!window.confirm(`Hapus fasilitas "${name}"?`)) return;
    void withStructureError(() => tfpGlidepathService.deleteFacility(record!.id, facilityId));
  };
  ...
  const handleMoveFacility = (idx: number, dir: -1 | 1) => {
    ...
    if (target < 0 || target >= ids.length) return;
    [ids[idx], ids[target]] = [ids[target], ids[idx]];
    void withStructureError(() => tfpGlidepathService.reorderFacilities(record.id, ids));
  };
  ...
  if (!record) {
    return (
      <div className="max-w-7xl mx-auto py-20 text-center">
        ...
        <Button onClick={() => navigate('/tfp/glidepath')} className="mt-4">
          Kembali ke Daftar
        </Button>
      </div>
    );
  ...
  return (
    <div className="max-w-full space-y-6 animate-fade-in pb-20">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <button type="button" onClick={() => navigate('/tfp/glidepath')} className="inline-flex items-center gap-1 hover:text-slate-700 transition-colors">Performance Check Gedung Glide Path</button>
        <span>/</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <Button variant="ghost" size="sm" onClick={() => navigate('/tfp/glidepath')} className="hover:bg-slate-100 mt-0.5">
              <ArrowLeft size={20} />
            </Button>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">Performance Check Gedung Glide Path</h1>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm"
              onClick={() => navigate(`/tfp/glidepath/${record.id}/print`)}
