...
import type { TfpGlidepathRecordDetail, TfpGlidepathItem } from '@/types/tfpGlidepath';

const isDisabled = (item: TfpGlidepathItem): boolean => item.is_disabled_map?.['panel_gp01'] === true;
...
export const TfpGlidepathDetailPage: React.FC = () => {
  ...
  const fetchRecord = useCallback(async () => {
    ...
    try {
      ...
      data.items.forEach((item) => { iv[item.id] = item.panel_gp01 ?? ''; });
      setItemValues(iv);
  ...
  const handleSave = async () => {
    ...
    try {
      const itemsPayload = record.items.map((item) => ({ id: item.id, panel_gp01: itemValues[item.id] ?? null }));
      ...
      const updated = await tfpGlidepathService.updateRecord(record.id, { items: itemsPayload, facilities: facilitiesPayload });
      setRecord(updated);
  ...
  if (!record) return (
    <div className="max-w-7xl mx-auto py-20 text-center">
      ...
      <Button onClick={() => navigate('/tfp/glidepath')} className="mt-4">Kembali ke Daftar</Button>
    </div>
  );
  ...
  return (
    <div className="max-w-full space-y-6 animate-fade-in pb-20">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <Button variant="ghost" size="sm" onClick={() => navigate('/tfp/glidepath')} className="hover:bg-slate-100 mt-0.5"><ArrowLeft size={20} /></Button>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">Performance Check Fasilitas Penunjang Gedung Glide Path</h1>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/tfp/glidepath/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50"><Printer size={15} />Print</Button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-6 items-start">
        {/* Parameter table — single column Panel GP 01 */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse" style={{ tableLayout: 'fixed', minWidth: '400px' }}>
              <colgroup>
                <col style={{ width: '28px' }} /><col style={{ width: '200px' }} /><col style={{ width: '120px' }} />
              </colgroup>
              <thead>
                <tr className="bg-sky-800 text-white">
                  <th className="px-1 py-2 text-center font-semibold border border-sky-700 text-xs">No</th>
                  <th className="px-2 py-2 text-left font-semibold border border-sky-700 text-xs">Parameter</th>
                  <th className="px-1 py-2 text-center font-semibold border border-sky-700 text-[10px]">Panel GP 01</th>
                </tr>
              </thead>
              <tbody>
                {record.items.map((item, idx) => {
                  ...
                  return (
                    <tr key={item.id} className={rowBase}>
                      ...
                      <td className="px-2 py-1.5 font-medium text-slate-700 text-xs border-r border-slate-100">
                        {item.parameter_name}{item.unit && <span className="text-slate-400 ml-1 text-[10px]">({item.unit})</span>}
                      </td>
                      <td className={cn('px-1.5 py-1 border-l border-slate-100', disabled ? 'bg-slate-200' : '')}>
                        {disabled ? (
                          <div className="w-full h-7 bg-slate-200 rounded" aria-hidden="true" />
                        ) : isCompleted ? (
      ...
      <TfpGlidepathSignaturePanel record={record} onUpdated={(updated) => {
        setRecord(updated);
        const iv: Record<number, string> = {};
        updated.items.forEach((item) => { iv[item.id] = item.panel_gp01 ?? ''; });
        setItemValues(iv);
      }} />
    </div>
  );
