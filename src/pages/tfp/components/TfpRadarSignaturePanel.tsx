...
import type { TfpRadarRecordDetail, TfpRadarRoleKey, TfpRadarSignerInfo, TfpRadarTechnicianRow } from '@/types/tfpRadar';

interface Props {
  record: TfpRadarRecordDetail;
  onUpdated: (record: TfpRadarRecordDetail) => void;
}
...
export const TfpRadarSignaturePanel: React.FC<Props> = ({ record, onUpdated }) => {
  const { user } = useAuth();
  ...
  type Column =
    | { kind: 'manager' | 'supervisor'; label: string; info: TfpRadarSignerInfo | null }
    | { kind: 'technician'; label: string; row: TfpRadarTechnicianRow };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await TfpRadarService.signRecord(record.id, pending.role, signature, pending.technicianRowId);
