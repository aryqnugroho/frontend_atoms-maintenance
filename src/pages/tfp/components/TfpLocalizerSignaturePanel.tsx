...
import type { TfpLocalizerRecordDetail, TfpLocalizerRoleKey, TfpLocalizerSignerInfo, TfpLocalizerTechnicianRow } from '@/types/tfpLocalizer';

interface Props {
  record: TfpLocalizerRecordDetail;
  onUpdated: (record: TfpLocalizerRecordDetail) => void;
}
...
export const TfpLocalizerSignaturePanel: React.FC<Props> = ({ record, onUpdated }) => {
  const { user } = useAuth();
  ...
  type Column =
    | { kind: 'manager' | 'supervisor'; label: string; info: TfpLocalizerSignerInfo | null }
    | { kind: 'technician'; label: string; row: TfpLocalizerTechnicianRow };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await tfpLocalizerService.signRecord(record.id, pending.role, signature, pending.technicianRowId);
