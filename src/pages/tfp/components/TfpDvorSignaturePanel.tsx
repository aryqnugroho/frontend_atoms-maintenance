...
import type { TfpDvorRecordDetail, TfpDvorRoleKey, TfpDvorSignerInfo, TfpDvorTechnicianRow } from '@/types/tfpDvor';

interface Props {
  record: TfpDvorRecordDetail;
  onUpdated: (record: TfpDvorRecordDetail) => void;
}
...
export const TfpDvorSignaturePanel: React.FC<Props> = ({ record, onUpdated }) => {
  const { user } = useAuth();
  ...
  type Column =
    | { kind: 'manager' | 'supervisor'; label: string; info: TfpDvorSignerInfo | null }
    | { kind: 'technician'; label: string; row: TfpDvorTechnicianRow };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await tfpDvorService.signRecord(record.id, pending.role, signature, pending.technicianRowId);
