...
import type {
  ...
  TfpAobLt12RoleKey,
  TfpAobLt12SignerInfo,
  TfpAobLt12TechnicianRow,
} from '@/types/tfpAobLt12';

interface Props {
  record: TfpAobLt12RecordDetail;
  onUpdated: (record: TfpAobLt12RecordDetail) => void;
}
...
export const TfpAobLt12SignaturePanel: React.FC<Props> = ({ record, onUpdated }) => {
  const { user } = useAuth();
  ...
  type Column =
    | { kind: 'manager' | 'supervisor'; label: string; info: TfpAobLt12SignerInfo | null }
    | { kind: 'technician'; label: string; row: TfpAobLt12TechnicianRow };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await tfpAobLt12Service.signRecord(record.id, pending.role, signature, pending.technicianRowId);
