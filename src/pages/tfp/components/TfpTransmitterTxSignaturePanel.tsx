...
import type { TfpTransmitterTxRecordDetail, TfpTransmitterTxRoleKey, TfpTransmitterTxSignerInfo, TfpTransmitterTxTechnicianRow } from '@/types/tfpTransmitterTx';
...
export const TfpTransmitterTxSignaturePanel: React.FC<Props> = ({ record, onUpdated }) => {
  ...
  type Column =
    | { kind: 'manager' | 'supervisor'; label: string; info: TfpTransmitterTxSignerInfo | null }
    | { kind: 'technician'; label: string; row: TfpTransmitterTxTechnicianRow };
  ...
  const renderColumn = (col: Column, idx: number): React.ReactElement => {
    if (col.kind === 'manager' || col.kind === 'supervisor') {      const info = col.info;
