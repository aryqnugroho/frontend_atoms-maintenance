...
import type { TfpTowerRecordDetail, TfpTowerRoleKey, TfpTowerSignerInfo, TfpTowerTechnicianRow } from '@/types/tfpTower';

interface Props {
  record: TfpTowerRecordDetail;
  onUpdated: (record: TfpTowerRecordDetail) => void;
}
...
export const TfpTowerSignaturePanel: React.FC<Props> = ({ record, onUpdated }) => {
  const { user } = useAuth();
  ...
  type Column =
    | { kind: 'manager' | 'supervisor'; label: string; info: TfpTowerSignerInfo | null }
    | { kind: 'technician'; label: string; row: TfpTowerTechnicianRow };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await tfpTowerService.signRecord(record.id, pending.role, signature, pending.technicianRowId);
