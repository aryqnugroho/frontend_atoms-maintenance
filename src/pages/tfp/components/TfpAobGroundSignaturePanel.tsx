...
import type {
  ...
  TfpAobGroundRoleKey,
  TfpAobGroundSignerInfo,
  TfpAobGroundTechnicianRow,
} from '@/types/tfpAobGround';

interface Props {
  record: TfpAobGroundRecordDetail;
  onUpdated: (record: TfpAobGroundRecordDetail) => void;
}
 ...
 *   2. Supervisor TFP (single signature row)
 *   3. Pelaksana Teknisi TFP — one row per technician on duty for this shift
 *
 ...
 */
export const TfpAobGroundSignaturePanel: React.FC<Props> = ({ record, onUpdated }) => {
  const { user } = useAuth();
  ...
  type Column =
    | {
        ...
        label: string;
        info: TfpAobGroundSignerInfo | null;
      }
    | {
        kind: 'technician';
        label: string;
        row: TfpAobGroundTechnicianRow;
      };
  ...
  const canSignTechnician =
    userRole === 'Teknisi TFP' || userRole === 'Supervisor TFP' || userRole === 'Admin';
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await tfpAobGroundService.signRecord(
        record.id,
        pending.role,
        signature,
          ...
          <div key={`${colKind}-${idx}`} className="space-y-3">
            ...
            {!isSigned && isAuthorized && !isCompleted && (
              <Button
                ...
                onClick={() =>
                  setPending({ role: colKind, label: colLabel, signerName: expectedName })
                }
              >
                <PenLine size={16} />
                Tanda Tangan
