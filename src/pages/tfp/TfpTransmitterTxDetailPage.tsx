...
import type { TfpTransmitterTxRecordDetail, TfpTransmitterTxItem } from '@/types/tfpTransmitterTx';
...
const isDisabled = (item: TfpTransmitterTxItem, colKey: ItemColKey): boolean =>
  item.is_disabled_map?.[colKey] === true;

const isModeRow = (item: TfpTransmitterTxItem): boolean =>
  item.parameter_name.toLowerCase().startsWith('mode');

const isSuplaiRow = (item: TfpTransmitterTxItem): boolean =>
  item.parameter_name.toLowerCase().startsWith('suplai aktif');

const isSingleValueRow = (item: TfpTransmitterTxItem): boolean => {
  const n = item.parameter_name.toLowerCase();
  return n.startsWith('suhu ruangan') || n.startsWith('kwh');
};

interface CellInputProps {
  item: TfpTransmitterTxItem;
  colKey: ItemColKey;
  value: string;
...
export const TfpTransmitterTxDetailPage: React.FC = () => {
  ...
  const fetchRecord = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const data = await tfpTransmitterTxService.getRecord(Number(id));
      setRecord(data);
      const iv: Record<number, Record<ItemColKey, string>> = {};
      data.items.forEach((item) => {
        iv[item.id] = {
          panel_tx01: item.panel_tx01 ?? '', panel_tx02: item.panel_tx02 ?? '',
          panel_cos_tx03_input: item.panel_cos_tx03_input ?? '', panel_cos_tx03_output: item.panel_cos_tx03_output ?? '',
          panel_output_ups_tx04: item.panel_output_ups_tx04 ?? '',
          panel_ups_tx07_input: item.panel_ups_tx07_input ?? '', panel_ups_tx07_output: item.panel_ups_tx07_output ?? '',
          panel_ac_tx06: item.panel_ac_tx06 ?? '',
          ups_piller_input: item.ups_piller_input ?? '', ups_piller_output: item.ups_piller_output ?? '',
          panel_milat_ru11: item.panel_milat_ru11 ?? '',
        };
      });
      setItemValues(iv);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await tfpTransmitterTxService.updateRecord(record.id, { items: itemsPayload, facilities: facilitiesPayload });
      setRecord(updated);
  ...
  if (!record) return (
    <div className="max-w-7xl mx-auto py-20 text-center">
      ...
      <Button onClick={() => navigate('/tfp/transmitter-tx')} className="mt-4">Kembali ke Daftar</Button>
    </div>
  );
  ...
  return (
    <div className="max-w-full space-y-6 animate-fade-in pb-20">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <button type="button" onClick={() => navigate('/tfp/transmitter-tx')} className="inline-flex items-center gap-1 hover:text-slate-700 transition-colors">Performance Check Transmitter TX</button>
        <span>/</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <Button variant="ghost" size="sm" onClick={() => navigate('/tfp/transmitter-tx')} className="hover:bg-slate-100 mt-0.5"><ArrowLeft size={20} /></Button>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">Performance Check Gedung (Transmitter) TX</h1>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/tfp/transmitter-tx/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50"><Printer size={15} />Print</Button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-6 items-start">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse" style={{ tableLayout: 'fixed', minWidth: '900px' }}>
              <colgroup>
                <col style={{ width: '28px' }} /><col style={{ width: '140px' }} />
              ...
              <thead>
                <tr className="bg-sky-800 text-white">
                  ...
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[10px]">Panel TX 01</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[10px]">Panel TX 02</th>
                  <th colSpan={2} className="px-1 py-1 text-center font-semibold border border-sky-700 text-[10px]">Panel COS (TX 03)</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[10px]">Panel Output UPS TX 04</th>
                  <th colSpan={2} className="px-1 py-1 text-center font-semibold border border-sky-700 text-[10px]">Panel UPS (TX 07)</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[10px]">Panel AC TX 06</th>
                  <th colSpan={2} className="px-1 py-1 text-center font-semibold border border-sky-700 text-[10px]">UPS PILLER</th>
                  <th rowSpan={2} className="px-1 py-2 text-center font-semibold border border-sky-700 text-[10px]">Panel MILAT RU 11</th>
                </tr>
                <tr className="bg-sky-700 text-sky-100">
                  {['Input','Output','Input','Output','Input','Output'].map((lbl, i) => (
                    <th key={i} className="px-1 py-1 text-center font-medium border border-sky-600 text-[10px]">{lbl}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {record.items.map((item, idx) => {
                  ...
                  if (isModeRow(item)) {
                    const cosVal = itemValues[item.id]?.panel_cos_tx03_input ?? '';
                    const upsVal = itemValues[item.id]?.panel_ups_tx07_input ?? '';
                    return (
                      <tr key={item.id} className="bg-blue-50/40">
                        ...
                        <td colSpan={2} className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{cosVal || '—'}</span> : <select value={cosVal} onChange={(e) => setItemCell(item.id, 'panel_cos_tx03_input', e.target.value)} className={modeSelectCls(cosVal)}><option value="">—</option><option value="Auto">Auto</option><option value="Manual">Manual</option></select>}
                        </td>
                        <td className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                        <td colSpan={2} className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{upsVal || '—'}</span> : <select value={upsVal} onChange={(e) => setItemCell(item.id, 'panel_ups_tx07_input', e.target.value)} className={modeSelectCls(upsVal)}><option value="">—</option><option value="Auto">Auto</option><option value="Manual">Manual</option></select>}
                        </td>
                        <td className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                        <td className="px-1 py-1 border-l border-slate-100" /><td className="px-1 py-1 border-l border-slate-100" />
                        <td className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                      </tr>
                    );
                  }
                  ...
                  if (isSuplaiRow(item)) {
                    ...
                    return (
                      <tr key={item.id} className="bg-blue-50/40">
                        ...
                        <td className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                        <td colSpan={2} className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{cosVal || '—'}</span> : <select value={cosVal} onChange={(e) => setItemCell(item.id, 'panel_cos_tx03_input', e.target.value)} className={suplaiSelectCls(cosVal)}><option value="">—</option><option value="PLN">PLN</option><option value="UPS">UPS</option></select>}
                        </td>
                        <td className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                        <td colSpan={2} className="px-1.5 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{upsVal || '—'}</span> : <select value={upsVal} onChange={(e) => setItemCell(item.id, 'panel_ups_tx07_input', e.target.value)} className={suplaiSelectCls(upsVal)}><option value="">—</option><option value="PLN">PLN</option><option value="UPS">UPS</option></select>}
                        </td>
                        <td className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                        <td className="px-1 py-1 border-l border-slate-100" /><td className="px-1 py-1 border-l border-slate-100" />
                        <td className="px-1 py-1 border-l border-slate-100 bg-slate-200" />
                  ...
                  if (isSingleValueRow(item)) {
                    const val = itemValues[item.id]?.panel_tx01 ?? '';
                    return (
                      <tr key={item.id} className={rowBase}>
                        ...
                        <td colSpan={11} className="px-3 py-1 border-l border-slate-100">
                          {isCompleted ? <span className="text-xs text-slate-700">{val || '—'}</span> : <input type="text" inputMode="decimal" value={val} onChange={(e) => setItemCell(item.id, 'panel_tx01', e.target.value)} className="w-48 h-7 px-2 text-xs rounded border border-slate-200 bg-white focus:ring-1 focus:ring-sky-400 focus:outline-none" />}
                        </td>
                      </tr>
                    );
                  }
      ...
      <TfpTransmitterTxSignaturePanel record={record} onUpdated={(updated) => {
        setRecord(updated);
        const iv: Record<number, Record<ItemColKey, string>> = {};
        updated.items.forEach((item) => {
          iv[item.id] = { panel_tx01: item.panel_tx01 ?? '', panel_tx02: item.panel_tx02 ?? '', panel_cos_tx03_input: item.panel_cos_tx03_input ?? '', panel_cos_tx03_output: item.panel_cos_tx03_output ?? '', panel_output_ups_tx04: item.panel_output_ups_tx04 ?? '', panel_ups_tx07_input: item.panel_ups_tx07_input ?? '', panel_ups_tx07_output: item.panel_ups_tx07_output ?? '', panel_ac_tx06: item.panel_ac_tx06 ?? '', ups_piller_input: item.ups_piller_input ?? '', ups_piller_output: item.ups_piller_output ?? '', panel_milat_ru11: item.panel_milat_ru11 ?? '' };
        });
        setItemValues(iv);
      }} />
    </div>
