...
import {
  ArrowLeft, Save, Printer, Users, Calendar, Clock, Zap, CheckSquare,
  Pencil, Plus, Trash2, ChevronUp, ChevronDown, Settings2, X, Check,
  Ban, MoveHorizontal, Columns, ColumnsIcon, Eraser, AlertCircle,
} from 'lucide-react';
...
import type {
  ...
  TfpTransmitterTxItem,
  TfpTransmitterTxFacility,
  TfpTransmitterTxColumnsConfig,
  TfpTransmitterTxPanel,
...
const flattenColumns = (config: TfpTransmitterTxColumnsConfig): FlatCell[] => {
  const out: FlatCell[] = [];
  let i = 0;
  for (const panel of config) {
    for (const sub of panel.sub_columns) {
      out.push({
        panel, subKey: sub.key, subLabel: sub.label,
...
const EditCell: React.FC<EditCellProps> = ({ isDisabled, isSelected, colspan, onClick, label }) => (
  <button type="button" onClick={onClick}
    className={cn(
      ...
      isDisabled ? 'bg-slate-200 text-slate-400 hover:bg-slate-300' : 'bg-sky-50 text-sky-700 hover:bg-sky-100 border border-sky-200',
...
interface AddPanelModalProps {
  open: boolean;
  onClose: () => void;
  onAdd: (panel: TfpTransmitterTxPanel) => void;
  existingIds: string[];
}
...
export const TfpTransmitterTxDetailPage: React.FC = () => {
  ...
  const hydrate = (data: TfpTransmitterTxRecordDetail) => {
    setRecord(data);
  ...
  const effectiveConfig: TfpTransmitterTxColumnsConfig = useMemo(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await tfpTransmitterTxService.updateRecord(record.id, {
        items: itemsPayload,
  ...
  const handleSaveStructure = async () => {
    ...
    try {
      ...
      const updated = await tfpTransmitterTxService.saveStructure(record.id, {
  ...
  const mutateConfig = (fn: (cfg: TfpTransmitterTxColumnsConfig) => TfpTransmitterTxColumnsConfig) => {
    setDraftConfig((prev) => {
  ...
  const handleAddPanel = (panel: TfpTransmitterTxPanel) => {
    mutateConfig((cfg) => [...cfg, panel]);
  };

  // ─── Existing structure ops ────────────────────────────────────────────

  const withStructureError = async (fn: () => Promise<TfpTransmitterTxRecordDetail>) => {
    setErrorMessage(null);
    try {
  ...
  const handleAddParameter = (data: { parameter_name: string; parameter_number?: string | null; unit?: string | null }) =>
    withStructureError(() => tfpTransmitterTxService.addParameter(record!.id, data));

  const handleUpdateParameter = (paramId: number, patch: { parameter_number?: string | null; parameter_name?: string; unit?: string | null }) =>
    withStructureError(() => tfpTransmitterTxService.updateParameter(record!.id, paramId, patch));
  ...
  const handleDeleteParameter = (paramId: number, name: string) => {
    ...
    void withStructureError(() => tfpTransmitterTxService.deleteParameter(record!.id, paramId));
  };
  ...
  const handleMoveParameter = (idx: number, dir: -1 | 1) => {
    ...
    void withStructureError(() => tfpTransmitterTxService.reorderParameters(record.id, ids));
  };

  const handleAddFacility = (name: string) =>
    withStructureError(() => tfpTransmitterTxService.addFacility(record!.id, { facility_name: name }));

  const handleUpdateFacility = (facilityId: number, name: string) =>
    withStructureError(() => tfpTransmitterTxService.updateFacility(record!.id, facilityId, { facility_name: name }));

  const handleDeleteFacility = (facilityId: number, name: string) => {
    if (!window.confirm(`Hapus fasilitas "${name}"?`)) return;
    void withStructureError(() => tfpTransmitterTxService.deleteFacility(record!.id, facilityId));
  };
  ...
  const handleMoveFacility = (idx: number, dir: -1 | 1) => {
    ...
    if (target < 0 || target >= ids.length) return;
    [ids[idx], ids[target]] = [ids[target], ids[idx]];
    void withStructureError(() => tfpTransmitterTxService.reorderFacilities(record.id, ids));
  };
  ...
  if (!record) {
    return (
      <div className="max-w-7xl mx-auto py-20 text-center">
        ...
        <Button onClick={() => navigate('/tfp/transmitter-tx')} className="mt-4">
          Kembali ke Daftar
        </Button>
      </div>
    );
  ...
  return (
    <div className="max-w-full space-y-6 animate-fade-in pb-20">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <button type="button" onClick={() => navigate('/tfp/transmitter-tx')} className="inline-flex items-center gap-1 hover:text-slate-700 transition-colors">Performance Check Gedung Transmitter</button>
        <span>/</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <Button variant="ghost" size="sm" onClick={() => navigate('/tfp/transmitter-tx')} className="hover:bg-slate-100 mt-0.5">
              <ArrowLeft size={20} />
            </Button>
            <div>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            {canEditStructure && !isCompleted && (
              <button type="button"
                ...
                className={cn(
                  ...
                  editMode ? 'bg-amber-500 text-white border-amber-500 hover:bg-amber-600'
            ...
            <Button variant="ghost" size="sm"
              onClick={() => navigate(`/tfp/transmitter-tx/${record.id}/print`)}
                ...
                {record.items.map((item, idx) => {
                  ...
                  flatCells.forEach((cell) => {
                    ...
                    if ((modeRow || suplaiRow) && !disabled) {
                      ...
                      const options = modeRow ? ['Auto', 'Manual'] as const : ['PLN', 'UPS'] as const;
      ...
      <TfpTransmitterTxSignaturePanel record={record} onUpdated={hydrate} />
...
interface FacilityRowProps {
  facility: TfpTransmitterTxFacility;
  idx: number;
  total: number;
  isCompleted: boolean;
