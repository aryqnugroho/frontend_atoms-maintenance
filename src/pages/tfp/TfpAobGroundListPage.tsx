...
const getSignerStatus = (
  record: TfpAobGroundRecordSummary,
...
/**
 * TFP Performance Check AOB Lantai Ground — list page.
 *
 * Mirrors the CNSD Recorder Meter list pattern: search + filter chips + reset,
 * empty-state DB-empty vs filter-empty, signer status badge, single
 * "Buat Performance Check" button.
 */
export const TfpAobGroundListPage: React.FC = () => {
  ...
  // ── data state ─────────────────────────────────────────
  const [records, setRecords] = useState<TfpAobGroundRecordSummary[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await tfpAobGroundService.listRecords(params);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        setErrorMessage('Gagal memuat daftar Performance Check AOB Ground.');
      }
      setRecords([]);
      setTotalCount(0);
    } finally {
  ...
  const handleCreate = async () => {
    ...
    setErrorMessage(null);
    try {
      const record: TfpAobGroundRecordDetail = await tfpAobGroundService.createRecord({
        date: getCurrentShiftDate(),
        shift_type: getCurrentShiftType(),
      });
      navigate(`/tfp/aob-ground/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        ...
        setErrorMessage(data.message ?? 'Gagal membuat Performance Check AOB Ground.');
        return;
      }
      setErrorMessage('Gagal membuat Performance Check AOB Ground. Coba lagi.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm(
        'Hapus form Performance Check AOB Ground ini? Tindakan ini tidak dapat diurungkan.',
      )
    )
      return;
    try {
      await tfpAobGroundService.deleteRecord(id);
      void fetchRecords();
    } catch {
  ...
  if (yearFilter)
    activeFilters.push({
      key: 'year',
      label: `Tahun: ${yearFilter}`,
      clear: () => setYearFilter(''),
    });
  if (shiftFilter)
    activeFilters.push({
      key: 'shift',
      label: SHIFT_LABELS[shiftFilter] ?? shiftFilter,
      clear: () => setShiftFilter(''),
    });
  if (statusFilter)
    activeFilters.push({
      key: 'status',
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Performance Check AOB Lantai Ground</span>
      </div>
      ...
      <PageHeader
        ...
        title="Performance Check AOB Lantai Ground"
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              aria-label="Cari Performance Check AOB Ground"
            />
          </div>
          {hasActiveFilters && (
            <button
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            ...
            <p className="text-base font-semibold text-slate-700">
              Belum ada Performance Check AOB Lantai Ground
            </p>
            <p className="text-sm text-slate-400 max-w-md">
              Klik <strong>Buat Performance Check</strong> untuk membuat form baru untuk shift hari
              ini. Backend akan otomatis mengisi teknisi TFP, supervisor, dan manager teknik dari
              roster.
            </p>
            {canCreate && (
            ...
            <table className="w-full text-sm min-w-[900px]">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="text-left text-xs font-medium text-slate-500 uppercase tracking-wider px-6 py-3 bg-gray-50/80 w-28">
                    Anda
                  </th>
                  <th className="text-center text-xs font-medium text-slate-500 uppercase tracking-wider px-6 py-3 bg-gray-50/80 w-28">
                    Aksi
                  </th>
                </tr>
              </thead>
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr
                      key={r.id}
                      onClick={() => navigate(`/tfp/aob-ground/${r.id}`)}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault();
                          navigate(`/tfp/aob-ground/${r.id}`);
                        }
                      }}
                      tabIndex={0}
                      role="button"
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-slate-700">
                        {r.supervisor_name ? (
                          <Badge variant="tfp" className="text-xs">
                            {r.supervisor_name}
                          </Badge>
                        ) : (
                      ...
                      <td className="px-6 py-4 text-center">
                        ...
                        >
                          ...
                          <button
                            onClick={() => navigate(`/tfp/aob-ground/${r.id}/print`)}
