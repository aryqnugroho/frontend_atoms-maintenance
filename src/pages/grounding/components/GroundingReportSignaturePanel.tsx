...
import { groundingReportService, type GroundingReportRoleKey } from '@/services/groundingReportService';
import type { GroundingReportDetail, GroundingReportTechnician } from '@/types/grounding';

interface Props {
  record: GroundingReportDetail;
  onUpdated: (record: GroundingReportDetail) => void;
}
...
type Column =
  | { kind: 'manager' | 'supervisor'; label: string; info: GroundingReportDetail['manager'] | GroundingReportDetail['supervisor'] }
  | { kind: 'technician'; label: string; row: GroundingReportTechnician };

export const GroundingReportSignaturePanel: React.FC<Props> = ({ record, onUpdated }) => {
  const { user } = useAuth();
  ...
  const columns: Column[] = useMemo(() => {
    ...
    record.technicians.forEach((t, idx) => {
      cols.push({ kind: 'technician', label: `Pelaksana Teknisi ${idx + 1}`, row: t });
    });
    return cols;
  }, [record]);

  const canSignManager = userRole === 'Manager Teknik';
  const canSignSupervisor = userRole === 'Supervisor TFP' || userRole === 'Admin';
  const canSignTechnician = userRole === 'Teknisi TFP' || userRole === 'Supervisor TFP' || userRole === 'Admin';
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await groundingReportService.signRecord(record.id, pending.role, signature, pending.technicianRowId);
  ...
  const renderColumn = (col: Column, idx: number): React.ReactElement => {
    ...
    // technician
    const row = (col as { kind: 'technician'; label: string; row: GroundingReportTechnician }).row;
