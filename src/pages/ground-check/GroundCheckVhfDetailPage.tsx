...
import { GroundCheckVhfSignaturePanel } from './components/GroundCheckVhfSignaturePanel';
import type {
  GroundCheckVhfInputType,
  GroundCheckVhfItem,
  GroundCheckVhfPhoto,
...
interface MaintenanceItemEditState {
  toleransi: string;
...
const dropdownOptions = (type: GroundCheckVhfInputType): { value: string; label: string }[] => {
  switch (type) {
    ...
    case 'dropdown_completeness':
      return [
        { value: 'Lengkap', label: 'Lengkap' },
        { value: 'Tidak Lengkap', label: 'Tidak Lengkap' },
      ];
    case 'dropdown_normality':
      return [
...
const HasilPdCell: React.FC<HasilPdCellProps> = ({ item, value, onChange, disabled }) => {
  ...
  if (
    ...
    item.input_type === 'dropdown_completeness' ||
    item.input_type === 'dropdown_normality'
  ) {
    return (
      <select
        value={value}
...
export const GroundCheckVhfDetailPage: React.FC = () => {
  ...
  // Editable item values (Form 1 + Form 2)
  const [itemValues, setItemValues] = useState<Record<number, ItemEditState>>({});
  const [maintItemValues, setMaintItemValues] = useState<Record<number, MaintenanceItemEditState>>({});
  ...
  const fetchRecord = useCallback(async () => {
    ...
    try {
      ...
      const mv: Record<number, MaintenanceItemEditState> = {};
      data.maintenance_items.forEach((m) => {
        mv[m.id] = {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const maintPayload = record.maintenance_items
        .filter((m) => !m.is_section_header && !m.is_subsection_header)
        .map((m) => ({
          id: m.id,
          toleransi: maintItemValues[m.id]?.toleransi || null,
          interface_value: maintItemValues[m.id]?.interface_value || null,
          tx1_value: maintItemValues[m.id]?.tx1_value || null,
          tx2_value: maintItemValues[m.id]?.tx2_value || null,
          keterangan: maintItemValues[m.id]?.keterangan || null,
        }));

      const result = await groundCheckVhfService.update(record.id, {
        ...
        items: itemsPayload,
        maintenance_items: maintPayload,
      });
  ...
  const setMaintField = (itemId: number, field: keyof MaintenanceItemEditState, val: string) => {
    setMaintItemValues((prev) => ({
      ...prev,
      [itemId]: { ...prev[itemId], [field]: val },
    }));
  };
  ...
  const handlePhotoFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await groundCheckVhfService.uploadPhoto(record.id, file, photoCaption || null);
      setRecord(result.data);
      setPhotoCaption('');
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
  ...
  const handleDeletePhoto = async (photoId: number) => {
    ...
    setPhotoError(null);
    try {
      const result = await groundCheckVhfService.deletePhoto(record.id, photoId);
      setRecord(result.data);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        const data = err.response.data as { message?: string };
  ...
  if (!record) {
    return (
      <div className="max-w-7xl mx-auto py-20 text-center">
        ...
        <Button onClick={() => navigate('/ground-check/vhf')} className="mt-4">
          Kembali ke Daftar
        </Button>
      </div>
    );
  ...
  return (
    <div className="max-w-full space-y-5 animate-fade-in pb-20">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <button type="button" onClick={() => navigate('/ground-check/vhf')} className="hover:text-slate-700 transition-colors">VHF</button>
        <span>/</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">
                  Pengujian Berkala di Darat — VHF
                </h1>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/ground-check/vhf/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          ...
          <EditableMeta label="Lokasi Peralatan" value={equipmentLocation} onChange={setEquipmentLocation} disabled={isCompleted} placeholder="Contoh: GEDUNG TX, RX DAN TOWER" />
        </div>

        <div className="grid grid-cols-1 gap-4">
          <EditableMeta label="Fungsi Peralatan" value={equipmentFunction} onChange={setEquipmentFunction} disabled={isCompleted} placeholder="Contoh: TMA DIRECTOR PRIMARY" />
          <EditableMeta label="Data Teknis" value={technicalData} onChange={setTechnicalData} disabled={isCompleted} placeholder="Contoh: FREQ. 123.2 MHz" />
      ...
      {/* Tabs: Form 1 / Form 2 / Photos */}
      <Tabs
        items={[
          { key: 'pengujian',   label: 'Pengujian Berkala',       icon: <ClipboardCheck size={15} /> },
          { key: 'maintenance', label: 'Pemeliharaan Pencegahan', icon: <ClipboardList size={15} /> },
      ...
      {activeTab === 'maintenance' && (
        <Form2MaintenanceTable
          record={record}
          values={maintItemValues}
          setField={setMaintField}
          isCompleted={isCompleted}
          isSaving={isSaving}
          onSave={() => void handleSave()}
        />
...
interface Form1Props {
  record: GroundCheckVhfRecordDetail;
...
// ─── Form 2 Maintenance Table ──────────────────────────────────────────────

interface Form2Props {
  record: GroundCheckVhfRecordDetail;
  values: Record<number, MaintenanceItemEditState>;
  setField: (itemId: number, field: keyof MaintenanceItemEditState, val: string) => void;
  isCompleted: boolean;
  isSaving: boolean;
  onSave: () => void;
}

const Form2MaintenanceTable: React.FC<Form2Props> = ({ record, values, setField, isCompleted, isSaving, onSave }) => {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-5 py-3 border-b border-slate-100 flex flex-wrap items-center justify-between gap-2 bg-slate-50/60">
        <div>
          <h2 className="text-sm font-bold text-slate-800">Form 2 — Pelaksanaan Kegiatan Pemeliharaan Pencegahan</h2>
          <p className="text-[11px] text-slate-500 mt-0.5">
            Catat hasil pemeliharaan bulanan: pembersihan, pengukuran parameter dengan alat ukur, dan pemeriksaan tegangan.
          </p>
        </div>
        {!isCompleted && (
          <Button size="sm" className="gap-2" onClick={onSave} disabled={isSaving} isLoading={isSaving}>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-slate-50 text-slate-700">
              <th className="px-2 py-2 text-center font-semibold border border-slate-200 w-12 text-[11px] uppercase tracking-wider">No</th>
              <th className="px-3 py-2 text-left font-semibold border border-slate-200 min-w-[260px] text-[11px] uppercase tracking-wider">Uraian Kegiatan</th>
              <th className="px-2 py-2 text-center font-semibold border border-slate-200 min-w-[100px] text-[11px] uppercase tracking-wider">Toleransi</th>
              <th className="px-2 py-2 text-center font-semibold border border-slate-200 min-w-[110px] text-[11px] uppercase tracking-wider">Interface</th>
              <th className="px-2 py-2 text-center font-semibold border border-slate-200 min-w-[100px] text-[11px] uppercase tracking-wider">TX1</th>
              <th className="px-2 py-2 text-center font-semibold border border-slate-200 min-w-[100px] text-[11px] uppercase tracking-wider">TX2</th>
              <th className="px-2 py-2 text-center font-semibold border border-slate-200 min-w-[140px] text-[11px] uppercase tracking-wider">Keterangan</th>
            </tr>
          </thead>
          <tbody>
            {record.maintenance_items.map((m) => {
              // Section header (big banner row)
              if (m.is_section_header) {
                return (
                  <tr key={m.id} className="bg-slate-100">
                    <td colSpan={7} className="px-3 py-2 text-xs font-bold text-slate-800 border-b border-slate-200 uppercase tracking-wider">
                      {m.parameter_name}
                    </td>
                  </tr>
                );
              ...
              // Subsection header (a. Transmitter / b. Receiver under section 2)
              if (m.is_subsection_header) {
                return (
                  <tr key={m.id} className="bg-slate-50">
                    <td className="px-2 py-1.5 text-slate-400 font-mono text-center border-r border-slate-100">{m.item_code ?? ''}</td>
                    <td colSpan={6} className="px-3 py-1.5 font-semibold text-slate-700 italic border-b border-slate-200">
                      {m.parameter_name}
                    </td>
                  </tr>
                );
              ...
              return (
                <tr key={m.id} className="bg-white hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-3 py-1.5 font-medium text-slate-700 border-r border-slate-100">
                    {m.subsection_label && (
                      <span className="text-[10px] uppercase tracking-wider text-slate-400 mr-2">{m.subsection_label} ·</span>
                    )}
                    {m.parameter_name}
                  </td>
                  <td className="px-1 py-1 text-center border-r border-slate-100">
                    {isCompleted ? (
                      <span className="text-xs text-slate-700">{v?.toleransi || m.toleransi || '—'}</span>
                    ) : (
                      <input
                        type="text"
                        value={v?.toleransi ?? ''}
                        onChange={(e) => setField(m.id, 'toleransi', e.target.value)}
                        placeholder={m.toleransi ?? '—'}
                        className="w-full h-8 px-1.5 text-center text-xs rounded border border-slate-200 bg-white focus:ring-2 focus:ring-sky-400 focus:border-transparent focus:outline-none placeholder:text-slate-300"
                      />
                    )}
                  </td>
                  <td className="px-1 py-1 text-center border-r border-slate-100">
                    {isCompleted ? (
                      <span className="text-xs text-slate-700">{v?.interface_value || m.interface_value || '—'}</span>
                    ) : (
                      <input
                        type="text"
                        value={v?.interface_value ?? ''}
                        onChange={(e) => setField(m.id, 'interface_value', e.target.value)}
                        placeholder={m.interface_value ?? '—'}
                        className="w-full h-8 px-1.5 text-center text-xs rounded border border-slate-200 bg-white focus:ring-2 focus:ring-sky-400 focus:border-transparent focus:outline-none placeholder:text-slate-300"
                      />
                    )}
                  </td>
                  <td className="px-1 py-1 text-center border-r border-slate-100">
                    {isCompleted ? (
                      <span className="text-xs text-slate-700 font-mono">{v?.tx1_value || '—'}</span>
                    ) : (
                      <input
                        type="text"
                        inputMode={m.input_type === 'numeric' ? 'decimal' : 'text'}
                        value={v?.tx1_value ?? ''}
                        onChange={(e) => setField(m.id, 'tx1_value', e.target.value)}
                        placeholder="—"
                        className={`w-full h-8 px-1.5 text-center text-xs rounded border border-slate-200 bg-white focus:ring-2 focus:ring-sky-400 focus:border-transparent focus:outline-none placeholder:text-slate-300 ${m.input_type === 'numeric' ? 'font-mono' : ''}`}
                      />
                    )}
                  </td>
                  <td className="px-1 py-1 text-center border-r border-slate-100">
                    {isCompleted ? (
                      <span className="text-xs text-slate-700 font-mono">{v?.tx2_value || '—'}</span>
                    ) : (
                      <input
                        type="text"
                        inputMode={m.input_type === 'numeric' ? 'decimal' : 'text'}
                        value={v?.tx2_value ?? ''}
                        onChange={(e) => setField(m.id, 'tx2_value', e.target.value)}
                        placeholder="—"
                        className={`w-full h-8 px-1.5 text-center text-xs rounded border border-slate-200 bg-white focus:ring-2 focus:ring-sky-400 focus:border-transparent focus:outline-none placeholder:text-slate-300 ${m.input_type === 'numeric' ? 'font-mono' : ''}`}
                      />
                    )}
                  ...
                  <td className="px-1 py-1">
                    {isCompleted ? (
                      <span className="text-xs text-slate-700">{v?.keterangan || '—'}</span>
                    ) : (
                      <input
                        type="text"
                        value={v?.keterangan ?? ''}
                        onChange={(e) => setField(m.id, 'keterangan', e.target.value)}
                        className="w-full h-8 px-1.5 text-xs rounded border border-slate-200 bg-white focus:ring-2 focus:ring-sky-400 focus:border-transparent focus:outline-none"
                      />
                    )}
                  </td>
