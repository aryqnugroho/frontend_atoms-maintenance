...
import type { GroundCheckAdcRecordSummary } from '@/types/groundCheckAdc';
...
const getSignerStatus = (
  record: GroundCheckAdcRecordSummary,
...
export const GroundCheckAdcListPage: React.FC = () => {
  ...
  const [records, setRecords] = useState<GroundCheckAdcRecordSummary[]>([]);
  ...
  useEffect(() => {
    groundCheckAdcService.getYears().then(setAvailableYears).catch(() => setAvailableYears([new Date().getFullYear()]));
  }, []);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await groundCheckAdcService.list(params);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        setErrorMessage('Gagal memuat daftar Ground Check ADC.');
      }
      setRecords([]);
      setTotalCount(0);
    } finally {
  ...
  const handleCreate = async () => {
    ...
    setErrorMessage(null);
    try {
      const res = await groundCheckAdcService.create({ date: getCurrentShiftDate(), shift_type: getCurrentShiftType() });
      navigate(`/ground-check/adc/${res.data.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        ...
        if (err.response.status === 409 && data.existing_record) {
          navigate(`/ground-check/adc/${data.existing_record.id}`);
          return;
        }
        setErrorMessage(data.message ?? 'Gagal membuat Ground Check ADC.');
        return;
      }
      setErrorMessage('Gagal membuat Ground Check ADC. Coba lagi.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Hapus form Ground Check ADC ini? Tindakan ini tidak dapat diurungkan.')) return;
    try {
      await groundCheckAdcService.delete(id);
      void fetchRecords();
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">ADC</span>
      </div>
            ...
            <table className="w-full text-sm min-w-[900px]">
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr key={r.id} onClick={() => navigate(`/ground-check/adc/${r.id}`)} tabIndex={0} role="button" className="border-b border-gray-50 hover:bg-gray-50/50 cursor-pointer transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-primary">
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => navigate(`/ground-check/adc/${r.id}`)} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Detail / Edit"><Edit2 size={16} /></button>
                          <button onClick={() => navigate(`/ground-check/adc/${r.id}/print`)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Print"><Printer size={16} /></button>
