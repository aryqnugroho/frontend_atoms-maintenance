...
import { GroundCheckGpSignaturePanel } from './components/GroundCheckGpSignaturePanel';
import type {
  GroundCheckGpItem,
  GroundCheckGpPhoto,
...
export const GroundCheckGpDetailPage: React.FC = () => {
  ...
  const fetchRecord = useCallback(async () => {
    ...
    try {
      ...
      setNavOrganization(data.nav_organization ?? '');
  ...
  const handleSave = async () => {
    ...
    try {
      const itemsPayload = record.items
        ...
        .map((item) => {
          // For check-only items, don't send Hasil PD values
      ...
      const navPayload = record.nav_items
        ...
        .map((n) => ({
          id: n.id,
          tx1_value: navItemValues[n.id]?.tx1_value || null,
          tx2_value: navItemValues[n.id]?.tx2_value || null,
          keterangan: navItemValues[n.id]?.keterangan || null,
        }));

      const result = await groundCheckGpService.update(record.id, {
        ...
        nav_organization: navOrganization || null,
  ...
  const handlePhotoFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await groundCheckGpService.uploadPhoto(record.id, file, photoCaption || null);
      setRecord(result.data);
      setPhotoCaption('');
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
  ...
  const handleDeletePhoto = async (photoId: number) => {
    ...
    setPhotoError(null);
    try {
      const result = await groundCheckGpService.deletePhoto(record.id, photoId);
      setRecord(result.data);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        const data = err.response.data as { message?: string };
  ...
  if (!record) {
    return (
      <div className="max-w-7xl mx-auto py-20 text-center">
        ...
        <Button onClick={() => navigate('/ground-check/gp')} className="mt-4">
          Kembali ke Daftar
        </Button>
      </div>
    );
  ...
  return (
    <div className="max-w-full space-y-5 animate-fade-in pb-20">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <button type="button" onClick={() => navigate('/ground-check/gp')} className="hover:text-slate-700 transition-colors">Glide Path</button>
        <span>/</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">
                  Pengujian Berkala di Darat — ILS GLIDE PATH
                </h1>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/ground-check/gp/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          ...
          <EditableMeta label="Lokasi Peralatan" value={equipmentLocation} onChange={setEquipmentLocation} disabled={isCompleted} placeholder="Contoh: SHELTER GLIDE PATH" />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <EditableMeta label="Fungsi Peralatan" value={equipmentFunction} onChange={setEquipmentFunction} disabled={isCompleted} placeholder="Contoh: ALAT BANTU PENDARATAN" />
          <EditableMeta label="Data Teknis" value={technicalData} onChange={setTechnicalData} disabled={isCompleted} placeholder="Contoh: FREQUENSI 334.4 MHz" />
      ...
      {activeTab === 'nav' && (
        <NavAnalyzerSection
          ...
          navOrganization={navOrganization} setNavOrganization={setNavOrganization}
...
interface Form1Props {
  record: GroundCheckGpRecordDetail;
      ...
      <div className="px-5 py-3 border-b border-slate-100 flex flex-wrap items-center justify-between gap-2 bg-slate-50/60">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">
            Hasil Kalibrasi pre-filled dari standar (boleh diedit). <span className="font-medium text-slate-700">Hasil PD TX1/TX2</span> diisi manual. Baris check-only: hanya toggle IN/OUT TOL.
          </p>
        </div>
        {!isCompleted && (
        ...
        <table className="w-full text-xs border-collapse min-w-[960px]">
          <thead>
            <tr className="bg-slate-50 text-slate-700">
              ...
              <th rowSpan={3} className="px-2 py-2 text-center font-semibold border border-slate-200 align-middle min-w-[140px] text-[11px] uppercase tracking-wider">Toleransi</th>
...
const NavAnalyzerSection: React.FC<NavSectionProps> = ({
  ...
  navOrganization, setNavOrganization, navAnalyzerTitle, setNavAnalyzerTitle,
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 gap-3 text-xs">
          <InlineMetaField label="Organisasi" value={navOrganization} onChange={setNavOrganization} disabled={isCompleted} placeholder="PERUM LPPNPI CABANG SURABAYA" />
          <InlineMetaField label="Title NAV Analyzer" value={navAnalyzerTitle} onChange={setNavAnalyzerTitle} disabled={isCompleted} placeholder="GROUND CHECK GLIDE PATH DENGAN PIR ROHDE & SCHWARZ EVSG1000 NAV ANALYZER" />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/60">
          <h2 className="text-sm font-bold text-slate-800">Form 2 — Pengukuran NAV Analyzer</h2>
          <p className="text-[11px] text-slate-500 mt-0.5">
            Isi nilai TRANSMITER 1 dan TRANSMITER 2 untuk tiap parameter. Section "RCMS" = pembacaan di transmitter setting.
          </p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-xs border-collapse min-w-[700px]">
            <thead>
              <tr className="bg-slate-50 text-slate-700">
                ...
                <th className="px-3 py-2 text-left font-semibold border border-slate-200 min-w-[180px] text-[11px] uppercase tracking-wider">Parameter</th>
                <th className="px-2 py-2 text-center font-semibold border border-slate-200 min-w-[140px] text-[11px] uppercase tracking-wider">Transmiter 1</th>
                <th className="px-2 py-2 text-center font-semibold border border-slate-200 min-w-[140px] text-[11px] uppercase tracking-wider">Transmiter 2</th>
                <th className="px-2 py-2 text-center font-semibold border border-slate-200 min-w-[200px] text-[11px] uppercase tracking-wider">Keterangan</th>
              </tr>
            </thead>
            <tbody>
              {record.nav_items.map((n) => {
                if (n.is_section_header) {
                  return (
                    <tr key={n.id} className="bg-slate-100">
                      <td colSpan={5} className="px-3 py-2 text-xs font-bold text-slate-800 uppercase tracking-wider border-b border-slate-200">
                        {n.parameter_name}
                      </td>
                    </tr>
                  );
                ...
                return (
                  <tr key={n.id} className="bg-white hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-1 py-1 border-r border-slate-100">
                      {isCompleted ? (
                        ...
                        <input
                          type="text"
                          value={v?.tx1_value ?? ''}
                          onChange={(e) => setField(n.id, 'tx1_value', e.target.value)}
                          placeholder="—"
                          className="w-full h-8 px-1.5 text-center text-xs font-mono rounded border border-slate-200 bg-white focus:ring-2 focus:ring-sky-400 focus:border-transparent focus:outline-none placeholder:text-slate-300"
                        />
                      )}
                    ...
                    <td className="px-1 py-1 border-r border-slate-100">
                      {isCompleted ? (
                        ...
                        <input
                          type="text"
                          value={v?.tx2_value ?? ''}
                          onChange={(e) => setField(n.id, 'tx2_value', e.target.value)}
                          placeholder="—"
                          className="w-full h-8 px-1.5 text-center text-xs font-mono rounded border border-slate-200 bg-white focus:ring-2 focus:ring-sky-400 focus:border-transparent focus:outline-none placeholder:text-slate-300"
                        />
                      )}
                    ...
                    <td className="px-1 py-1">
                      {isCompleted ? (
                        <span className="text-xs text-slate-700">{v?.keterangan || n.section_keterangan || '—'}</span>
                      ) : (
                        <input
                          type="text"
                          value={v?.keterangan ?? ''}
                          onChange={(e) => setField(n.id, 'keterangan', e.target.value)}
                          placeholder={n.section_keterangan ?? ''}
                          className="w-full h-8 px-1.5 text-xs rounded border border-slate-200 bg-white focus:ring-2 focus:ring-sky-400 focus:border-transparent focus:outline-none placeholder:text-slate-300"
                        />
                      )}
                    </td>
  ...
  <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
    <div className="flex items-start justify-between gap-3 flex-wrap">
      <div>
        ...
        <p className="text-[11px] text-slate-500 mt-0.5">
          Lampirkan foto kegiatan ground check (shelter glide path, antena, monitoring, NAV analyzer, dll).
    ...
    {canEdit && (
      <div className="flex flex-col sm:flex-row gap-2">
        <input
          ...
          placeholder="Caption foto (opsional) — contoh: Shelter Glide Path dengan NAV Analyzer"
          maxLength={255}
