...
import type { GroundCheckDvorRecordSummary } from '@/types/groundCheckDvor';
...
const getSignerStatus = (
  record: GroundCheckDvorRecordSummary,
...
export const GroundCheckDvorListPage: React.FC = () => {
  ...
  const [records, setRecords] = useState<GroundCheckDvorRecordSummary[]>([]);
  ...
  useEffect(() => {
    groundCheckDvorService.getYears().then(setAvailableYears).catch(() => setAvailableYears([new Date().getFullYear()]));
  }, []);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await groundCheckDvorService.list(params);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        setErrorMessage('Gagal memuat daftar Ground Check DVOR.');
      }
      setRecords([]);
      setTotalCount(0);
    } finally {
  ...
  const handleCreate = async () => {
    ...
    setErrorMessage(null);
    try {
      const res = await groundCheckDvorService.create({ date: getCurrentShiftDate(), shift_type: getCurrentShiftType() });
      navigate(`/ground-check/dvor/${res.data.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        ...
        if (err.response.status === 409 && data.existing_record) {
          navigate(`/ground-check/dvor/${data.existing_record.id}`);
          return;
        }
        setErrorMessage(data.message ?? 'Gagal membuat Ground Check DVOR.');
        return;
      }
      setErrorMessage('Gagal membuat Ground Check DVOR. Coba lagi.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Hapus form Ground Check DVOR ini? Tindakan ini tidak dapat diurungkan.')) return;
    try {
      await groundCheckDvorService.delete(id);
      void fetchRecords();
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">DVOR</span>
      </div>
      ...
      <PageHeader
        ...
        title="Ground Check DVOR"
        subtitle="Pengujian Berkala di Darat & NAV Analyzer — DVOR"
        actions={canCreate && (
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            ...
            <p className="text-base font-semibold text-slate-700">Belum ada Ground Check DVOR</p>
            ...
            <table className="w-full text-sm min-w-[900px]">
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr key={r.id} onClick={() => navigate(`/ground-check/dvor/${r.id}`)} tabIndex={0} role="button" className="border-b border-gray-50 hover:bg-gray-50/50 cursor-pointer transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-primary">
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => navigate(`/ground-check/dvor/${r.id}`)} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Detail / Edit"><Edit2 size={16} /></button>
                          <button onClick={() => navigate(`/ground-check/dvor/${r.id}/print`)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Print"><Printer size={16} /></button>
