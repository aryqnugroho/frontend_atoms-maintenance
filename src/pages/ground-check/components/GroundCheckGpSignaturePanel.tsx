...
import type {
  ...
  GroundCheckGpTechnician,
} from '@/types/groundCheckGp';

interface Props {
  record: GroundCheckGpRecordDetail;
  onSignSuccess: (record: GroundCheckGpRecordDetail) => void;
}
...
export const GroundCheckGpSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await groundCheckGpService.sign(record.id, {
        role: pending.role,
        signature,
