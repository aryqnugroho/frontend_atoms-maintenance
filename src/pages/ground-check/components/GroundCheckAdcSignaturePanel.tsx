...
import type {
  ...
  GroundCheckAdcTechnician,
} from '@/types/groundCheckAdc';

interface Props {
  record: GroundCheckAdcRecordDetail;
  onSignSuccess: (record: GroundCheckAdcRecordDetail) => void;
}

type SignRole = 'manager' | 'supervisor' | 'technician';
...
/**
 * Ground Check ADC signature panel.
 *
 * Renders three blocks:
 *   1. Manager Teknik (single signature row)
 *   2. Supervisor (single signature row)
 *   3. Teknisi Pelaksana — one row per technician on duty
 *
 ...
 */
export const GroundCheckAdcSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
    ...
    const cols: Column[] = [
      {
        ...
        signature: record.manager_signature,
        signedAt: record.manager_signed_at,
      },
      {
        kind: 'supervisor',
        label: 'Supervisor',
        name: record.supervisor_name,
        signature: record.supervisor_signature,
        signedAt: record.supervisor_signed_at,
      },
    ];
    record.technicians.forEach((t, idx) => {
      cols.push({
        kind: 'technician',
        label: `Teknisi Pelaksana ${idx + 1}`,
        row: t,
      });
    });
    return cols;
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await groundCheckAdcService.sign(record.id, {
        role: pending.role,
        signature,
        technician_row_id: pending.technicianRowId,
      });
      onSignSuccess(result.data);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        setErrorMessage(
          resolveValidationMessage(err.response.data) ?? 'Gagal menyimpan tanda tangan.',
        );
      } else {
        ...
        return (
          <div key={`${col.kind}-${idx}`} className="space-y-3">
            <SignatureDisplay
              ...
              signedAt={col.signedAt}
              signatureImage={col.signature}
              role={col.label}
        ...
        return (
          <div key={`tech-${row.id}`} className="space-y-3">
            <SignatureDisplay
              ...
              signedAt={row.technician_signed_at}
              signatureImage={row.technician_signature}
              role={col.label}
              isPending={!isSigned}
            />
            {!isSigned && isAuthorized && !isCompleted && (
