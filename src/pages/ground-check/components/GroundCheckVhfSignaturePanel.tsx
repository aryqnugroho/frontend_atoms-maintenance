...
import type {
  ...
  GroundCheckVhfTechnician,
} from '@/types/groundCheckVhf';

interface Props {
  record: GroundCheckVhfRecordDetail;
  onSignSuccess: (record: GroundCheckVhfRecordDetail) => void;
}
...
export const GroundCheckVhfSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await groundCheckVhfService.sign(record.id, {
        role: pending.role,
        signature,
