...
import type {
  ...
  GroundCheckLlzTechnician,
} from '@/types/groundCheckLlz';

interface Props {
  record: GroundCheckLlzRecordDetail;
  onSignSuccess: (record: GroundCheckLlzRecordDetail) => void;
}
...
export const GroundCheckLlzSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await groundCheckLlzService.sign(record.id, {
        role: pending.role,
        signature,
