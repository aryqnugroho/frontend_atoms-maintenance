...
import type {
  ...
  GroundCheckDvorTechnician,
} from '@/types/groundCheckDvor';

interface Props {
  record: GroundCheckDvorRecordDetail;
  onSignSuccess: (record: GroundCheckDvorRecordDetail) => void;
}
...
export const GroundCheckDvorSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await groundCheckDvorService.sign(record.id, {
        role: pending.role,
        signature,
