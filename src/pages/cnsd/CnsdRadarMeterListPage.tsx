...
const getSignerStatus = (
  record: CnsdRadarMeterRecordSummary,
...
/**
 * CNSD Radar Meter Reading list page.
 *
 * Mirrors the EQ-1 readiness list pattern: search + filter chips + reset,
 * empty-state DB-empty vs filter-empty, signer status badge, single
 * "Buat Meter Reading" button.
 */
export const CnsdRadarMeterListPage: React.FC = () => {
  ...
  // ── data state ─────────────────────────────────────────
  const [records, setRecords] = useState<CnsdRadarMeterRecordSummary[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await cnsdRadarMeterService.listRecords(params);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        setErrorMessage('Gagal memuat daftar Meter Reading Radar.');
      }
      setRecords([]);
      setTotalCount(0);
    } finally {
  ...
  const handleCreate = async () => {
    ...
    setErrorMessage(null);
    try {
      const record: CnsdRadarMeterRecordDetail = await cnsdRadarMeterService.createRecord({
        date: getCurrentShiftDate(),
        shift_type: getCurrentShiftType(),
      });
      navigate(`/cnsd/radar-meter/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        ...
        setErrorMessage(data.message ?? 'Gagal membuat Meter Reading Radar.');
        return;
      }
      setErrorMessage('Gagal membuat Meter Reading Radar. Coba lagi.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Hapus form Meter Reading Radar ini? Tindakan ini tidak dapat diurungkan.')) return;
    try {
      await cnsdRadarMeterService.deleteRecord(id);
      void fetchRecords();
    } catch {
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Meter Reading Radar</span>
      </div>
      ...
      <PageHeader
        ...
        title="Meter Reading Radar"
        subtitle="CNSD Equipment Readiness — Radar (ELDIS MSSR-1 / RL2000)"
        actions={
          canCreate && (
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              aria-label="Cari Meter Reading Radar"
            />
          </div>
          {hasActiveFilters && (
            <button
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            <div className="h-14 w-14 rounded-full bg-sky-50 flex items-center justify-center">
              <RadarIcon size={24} className="text-maintenance-cnsd/70" />
            </div>
            <p className="text-base font-semibold text-slate-700">Belum ada Meter Reading Radar</p>
            <p className="text-sm text-slate-400 max-w-md">
              Klik <strong>Buat Meter Reading</strong> untuk membuat form Radar baru untuk shift hari ini.
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr
                      key={r.id}
                      onClick={() => navigate(`/cnsd/radar-meter/${r.id}`)}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault();
                          navigate(`/cnsd/radar-meter/${r.id}`);
                        }
                      }}
                      tabIndex={0}
                      role="button"
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
                          ...
                          <button
                            onClick={() => navigate(`/cnsd/radar-meter/${r.id}/print`)}
