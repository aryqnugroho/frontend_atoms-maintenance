...
interface PrintSectionBlockProps {
  meta: CnsdVccsFreqMeterSectionMeta;
  items: CnsdVccsFreqMeterItem[];
}

/**
 * Render a single VCCS Freq section as table rows. Different layouts:
 *   - single_adaptive (FRONT PANEL): NO | PEMBACAAN | Standart | Redundant Server | KETERANGAN
 *   - dual_toggle_nf (MSC&RCMS / CWP): NO | PEMBACAAN | Indicator/Name | Normal | Fault | KETERANGAN
 *   - environment (LINGKUNGAN KERJA): NO | KEGIATAN | Nominal | HASIL PEMERIKSAAN | KETERANGAN
 */
    ...
    <>
      ...
      {layout === 'single_adaptive' && (
        <tr>
          ...
          <td colSpan={2} className="gc-group-bar text-center font-bold">
            {meta.columns_label_1 ?? 'Redundant Server'}
          </td>
      ...
      {layout === 'dual_toggle_nf' && (
        <tr>
          <td className="text-center font-bold gc-group-bar">{meta.code}</td>
          <td className="gc-group-bar font-bold uppercase">{meta.name}</td>
          <td className="gc-group-bar text-center font-bold">
            {meta.code === '3' ? 'Name' : 'Indicator'}
          </td>
            ...
            if (layout === 'single_adaptive') {
              return (
                <tr key={item.id} className="text-[10px]">
                  ...
                  <td colSpan={2} className="text-center">{text(item.hasil_a)}</td>
...
/**
 * CNSD VCCS Frequentis Meter Reading Print View — A4 portrait, B&W.
 *
 * Layout mirrors the official "METER READING — VCCS" Frequentis paper form:
 *   - Kop band: AirNav logo (left), Perum LPPNPI (right)
 *   - Title METER READING centered, VCCS gray bar middle
 *   - Metadata band: LOKASI/TGL JAM (left) · VCCS (center) · MERK/TYPE/SN (right)
 *   - Section A. PERALATAN umbrella bar + 3 sub-sections (FRONT PANEL / MSC & RCMS / CWP)
 *   - Section B. LINGKUNGAN KERJA
 *   - Footer 3-col: TEKNISI sub-table | SUPERVISOR | MANAGER TEKNIK
 *
 * Per SKILL.md: NO form_number on print. B&W only.
 */
export const CnsdVccsFreqMeterPrintView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRecord = async () => {
      try {
        const data = await cnsdVccsFreqMeterService.getRecord(Number(id));
        setRecord(data);
      } catch {
        setRecord(null);
      } finally {
  ...
  if (!record) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4">
        ...
        <Button variant="outline" onClick={() => navigate('/cnsd/vccs-freq-meter')}>Kembali</Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full bg-slate-100 p-4 text-black print:bg-white print:p-0">
      ...
      <div className="print-hide mx-auto mb-4 flex max-w-[210mm] items-center justify-between">
        <Button variant="outline" className="gap-2" onClick={() => navigate(`/cnsd/vccs-freq-meter/${record.id}`)}>
                ...
                <table className="gc-meta text-[10px]">
                  <tbody>
                    <tr>
                      ...
                      <td className="px-0.5">:</td>
                      <td>{text(record.merk) || 'FREQUENTIS'}</td>
                    </tr>
                    <tr>
