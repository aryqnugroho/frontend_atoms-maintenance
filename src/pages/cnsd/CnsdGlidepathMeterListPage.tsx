...
import type { CnsdGlidepathMeterRecordSummary, CnsdGlidepathMeterRecordDetail } from '@/types/cnsdGlidepath';
...
const getSignerStatus = (r: CnsdGlidepathMeterRecordSummary, name?: string, role?: string) => {
...
export const CnsdGlidepathMeterListPage: React.FC = () => {
  ...
  const [records, setRecords] = useState<CnsdGlidepathMeterRecordSummary[]>([]);
  ...
  useEffect(() => {
    cnsdGlidepathMeterService.getYears().then(setAvailableYears).catch(() => setAvailableYears([new Date().getFullYear()]));
  }, []);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const res = await cnsdGlidepathMeterService.listRecords(params);
      setRecords(res.data ?? []);
      setTotalCount(res.total ?? res.data?.length ?? 0);
      setErrorMessage(null);
    } catch (err) {
      setErrorMessage(axios.isAxiosError(err) && err.response?.status === 401 ? 'Sesi habis. Silakan login ulang.' : 'Gagal memuat daftar Meter Reading Glide Path.');
  ...
  const handleCreate = async () => {
    ...
    try {
      const record: CnsdGlidepathMeterRecordDetail = await cnsdGlidepathMeterService.createRecord({ date: getCurrentShiftDate(), shift_type: getCurrentShiftType() });
      navigate(`/cnsd/glidepath-meter/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        const data = err.response.data as { message?: string; errors?: { existing_record?: { id: number } } };
        if (err.response.status === 409 && data.errors?.existing_record) { navigate(`/cnsd/glidepath-meter/${data.errors.existing_record.id}`); return; }
        setErrorMessage(data.message ?? (err.response.status === 422 ? 'Tidak ada teknisi CNSD yang bertugas pada shift ini.' : 'Gagal membuat Meter Reading Glide Path.'));
        return;
      }
      setErrorMessage('Gagal membuat Meter Reading Glide Path. Coba lagi.');
    } finally { setIsCreating(false); }
  };

  const handleDelete = async (id: number) => {
    if (!confirm('Hapus form Meter Reading Glide Path ini?')) return;
    try { await cnsdGlidepathMeterService.deleteRecord(id); void fetchRecords(); } catch { alert('Gagal menghapus form.'); }
  };
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Meter Reading Glide Path</span>
      </div>

      <PageHeader icon={Radio} iconBg="bg-sky-100" iconColor="text-maintenance-cnsd"
        title="Meter Reading Glide Path" subtitle="CNSD Equipment Readiness — ILS Glide Path (ILS-GP)"
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            ...
            <p className="text-base font-semibold text-slate-700">Belum ada Meter Reading Glide Path</p>
            <p className="text-sm text-slate-400 max-w-md">Klik <strong>Buat Meter Reading</strong> untuk membuat form Glide Path baru untuk shift hari ini.</p>
            ...
            <table className="w-full text-sm min-w-[800px]">
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr key={r.id} onClick={() => navigate(`/cnsd/glidepath-meter/${r.id}`)} tabIndex={0} role="button"
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => navigate(`/cnsd/glidepath-meter/${r.id}`)} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Detail / Edit"><Edit2 size={16} /></button>
                          <button onClick={() => navigate(`/cnsd/glidepath-meter/${r.id}/print`)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Print PDF"><Printer size={16} /></button>
