...
import { MonitorCog, Plus, Search, X, Filter, Edit2, Trash2, Users, ArrowLeft, Printer, PenLine, CheckCheck } from 'lucide-react';
...
import type { CnsdAtcSystemMeterRecordSummary, CnsdAtcSystemMeterRecordDetail } from '@/types/cnsdAtcSystem';
...
const getSignerStatus = (r: CnsdAtcSystemMeterRecordSummary, name?: string, role?: string) => {
  if (!name || !role) return 'none';
  if (r.status === 'completed') return 'completed';
  if (role === 'Manager Teknik' && namesMatch(r.manager_name, name)) return 'pending';
  if ((role === 'Supervisor CNSD' || role === 'Admin') && namesMatch(r.supervisor_name, name)) return 'pending';
  if (role === 'Teknisi CNSD' || role === 'Supervisor CNSD') {
    if (r.technician_names.some((n) => namesMatch(n, name))) return 'pending';
  }
  return 'none';
};

export const CnsdAtcSystemMeterListPage: React.FC = () => {
  ...
  const [records, setRecords] = useState<CnsdAtcSystemMeterRecordSummary[]>([]);
  ...
  useEffect(() => {
    cnsdAtcSystemMeterService.getYears().then(setAvailableYears).catch(() => setAvailableYears([new Date().getFullYear()]));
  }, []);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const res = await cnsdAtcSystemMeterService.listRecords(params);
      setRecords(res.data ?? []); setTotalCount(res.total ?? res.data?.length ?? 0); setErrorMessage(null);
    } catch (err) {
      setErrorMessage(axios.isAxiosError(err) && err.response?.status === 401 ? 'Sesi habis. Silakan login ulang.' : 'Gagal memuat daftar Meter Reading ATC SYSTEM.');
  ...
  const handleCreate = async () => {
    ...
    try {
      const record: CnsdAtcSystemMeterRecordDetail = await cnsdAtcSystemMeterService.createRecord({ date: getCurrentShiftDate(), shift_type: getCurrentShiftType() });
      navigate(`/cnsd/atc-system-meter/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        const data = err.response.data as { message?: string; errors?: { existing_record?: { id: number } } };
        if (err.response.status === 409 && data.errors?.existing_record) { navigate(`/cnsd/atc-system-meter/${data.errors.existing_record.id}`); return; }
        setErrorMessage(data.message ?? (err.response.status === 422 ? 'Tidak ada teknisi CNSD yang bertugas pada shift ini.' : 'Gagal membuat Meter Reading ATC SYSTEM.'));
        return;
      }
      setErrorMessage('Gagal membuat Meter Reading ATC SYSTEM. Coba lagi.');
    } finally { setIsCreating(false); }
  };

  const handleDelete = async (id: number) => {
    if (!confirm('Hapus form Meter Reading ATC SYSTEM ini?')) return;
    try { await cnsdAtcSystemMeterService.deleteRecord(id); void fetchRecords(); } catch { alert('Gagal menghapus form.'); }
  };
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Meter Reading ATC SYSTEM</span>
      </div>

      <PageHeader icon={MonitorCog} iconBg="bg-sky-100" iconColor="text-maintenance-cnsd"
        title="Meter Reading ATC SYSTEM" subtitle="CNSD Equipment Readiness — Approach System / Tern ATS System"
        actions={canCreate && <Button onClick={handleCreate} className="gap-2 shrink-0" isLoading={isCreating}><Plus size={16} /> Buat Meter Reading</Button>}
      />
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 space-y-3">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Cari nomor form, manager, supervisor..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-9 pr-3 h-10 rounded-lg border border-gray-300 bg-white text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-transparent" aria-label="Cari" />
          </div>
          {hasActiveFilters && <button onClick={resetFilters} className="h-10 flex items-center gap-1.5 px-3 rounded-lg border border-gray-300 text-sm text-slate-600 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors shrink-0"><X size={14} /><span className="hidden sm:inline">Reset</span></button>}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1">
          {hasActiveFilters && (<><Filter size={13} className="text-slate-400 shrink-0" />
            {activeFilters.map((f) => (
              <span key={f.key} className="inline-flex items-center gap-1 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-xs font-medium px-2.5 py-0.5">
                {f.label}<button onClick={f.clear} className="ml-0.5 rounded-full hover:bg-blue-100 transition-colors" aria-label={`Hapus filter ${f.label}`}><X size={11} /></button>
              </span>
            ))}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            <div className="h-14 w-14 rounded-full bg-sky-50 flex items-center justify-center"><MonitorCog size={24} className="text-maintenance-cnsd/70" /></div>
            <p className="text-base font-semibold text-slate-700">Belum ada Meter Reading ATC SYSTEM</p>
            <p className="text-sm text-slate-400 max-w-md">Klik <strong>Buat Meter Reading</strong> untuk membuat form ATC SYSTEM baru untuk shift hari ini.</p>
            {canCreate && <Button onClick={handleCreate} className="gap-2 mt-2" isLoading={isCreating}><Plus size={15} /> Buat Meter Reading</Button>}
          </div>
        ) : isFilterEmpty ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            ...
            <table className="w-full text-sm min-w-[800px]">
              <thead>
                <tr className="border-b border-gray-100">
                  {['No. Form', 'Tanggal', 'Shift', 'Teknisi', 'Supervisor', 'Manager', 'Status', 'Anda', 'Aksi'].map((h) => (
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr key={r.id} onClick={() => navigate(`/cnsd/atc-system-meter/${r.id}`)} tabIndex={0} role="button"
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => navigate(`/cnsd/atc-system-meter/${r.id}`)} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Detail / Edit"><Edit2 size={16} /></button>
                          <button onClick={() => navigate(`/cnsd/atc-system-meter/${r.id}/print`)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Print PDF"><Printer size={16} /></button>
