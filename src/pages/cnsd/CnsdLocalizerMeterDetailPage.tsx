...
/**
 * CNSD Localizer Meter Reading — detail / edit page (ILS — LOCALIZER).
 *
 * Mirrors paper 008_Localizer. Identical structure to Glide Path:
 *   - Section A (PERALATAN) with 8 groups using per-item single/dual hasil
 *     (FRONT PANEL uses TX1/TX2; CL, DS, CLR, Near Field use M1/M2;
 *      Power Supply groups use single HASIL).
 *   - Section B (LINGKUNGAN KERJA): standard NOMINAL | HASIL | KETERANGAN.
 */
export const CnsdLocalizerMeterDetailPage: React.FC = () => {
  ...
  const [editedItems, setEditedItems] = useState<Record<number, Partial<CnsdLocalizerMeterItem>>>({});
  ...
  const fetchRecord = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const data = await cnsdLocalizerMeterService.getRecord(recordId);
      setRecord(data);
      setEditedItems({});
      setMerk(data.merk ?? '');
      setType(data.type ?? '');
  ...
  const updateField = (itemId: number, field: keyof CnsdLocalizerMeterItem, value: string | null) => {
  ...
  const getValue = (item: CnsdLocalizerMeterItem, field: keyof CnsdLocalizerMeterItem): string => {
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await cnsdLocalizerMeterService.updateRecord(record.id, {
  ...
  if (!record) {
    return (
      <div className="max-w-3xl mx-auto py-20 text-center space-y-4">
        ...
        <Button variant="outline" onClick={() => navigate('/cnsd/localizer-meter')} className="gap-2">
  ...
  return (
    <div className="max-w-full space-y-5 animate-fade-in pb-12">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <button type="button" onClick={() => navigate('/cnsd/localizer-meter')} className="hover:text-slate-700 transition-colors">
          Localizer Meter Reading
        </button>
        <span>/</span>
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className="h-10 w-10 rounded-lg bg-sky-100 flex items-center justify-center shrink-0">
              <LocalizerIcon size={18} className="text-sky-600" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">Meter Reading — ILS Localizer</h1>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/cnsd/localizer-meter/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs pt-2 border-t border-slate-100">
          ...
          <EditableMetaField label="Type" value={type} onChange={setType} disabled={!!isReadOnly || !canEditMetadata} placeholder="500" />
      ...
      <CnsdLocalizerMeterSignaturePanel record={record} onSignSuccess={(r) => setRecord(r)} />
    </div>
  );
};

interface LocalizerSectionPanelProps {
  sectionMeta: SectionMeta;
  items: CnsdLocalizerMeterItem[];
  isReadOnly: boolean;
  getValue: (item: CnsdLocalizerMeterItem, field: keyof CnsdLocalizerMeterItem) => string;
  onChange: (itemId: number, field: keyof CnsdLocalizerMeterItem, value: string | null) => void;
}

const LocalizerSectionPanel: React.FC<LocalizerSectionPanelProps> = ({
  ...
  const groups = useMemo(() => {
    ...
    return order.map((key) => {
      ...
      return {
        number: first?.group_number ?? null,
      ...
      <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between gap-2 bg-slate-50/60">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">
            {isMeterReading
              ? 'FRONT PANEL menggunakan TX1/TX2; CL/DS/CLR/Near Field menggunakan M1/M2; Power Supply menggunakan satu kolom HASIL.'
...
interface LocalizerItemRowProps {
  item: CnsdLocalizerMeterItem;
  isReadOnly: boolean;
  getValue: (item: CnsdLocalizerMeterItem, field: keyof CnsdLocalizerMeterItem) => string;
  onChange: (itemId: number, field: keyof CnsdLocalizerMeterItem, value: string | null) => void;
}

const LocalizerItemRow: React.FC<LocalizerItemRowProps> = ({ item, isReadOnly, getValue, onChange }) => {
