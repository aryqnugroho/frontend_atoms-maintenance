...
function getDropdownOptions(item: CnsdLocalizerMeterItem): string[] | null {
...
export const CnsdLocalizerMeterDetailPage: React.FC = () => {
  ...
  const [localItems, setLocalItems] = useState<CnsdLocalizerMeterItem[]>([]);
  ...
  const fetchRecord = useCallback(async () => {
    ...
    try {
      ...
      setErrorMessage('Gagal memuat data form Localizer.');
  ...
  const handleSignSuccess = (updated: CnsdLocalizerMeterRecordDetail) => {
    setRecord(updated); setLocalItems(updated.items);
  };

  // Grouped items for section A — unconditional (Rules of Hooks)
  const groupedItemsA = useMemo(() => {
    const groups: Map<number, CnsdLocalizerMeterItem[]> = new Map();
  ...
  const renderInput = (item: CnsdLocalizerMeterItem, field: 'hasil_1' | 'hasil_2') => {
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      ...
      <PageHeader icon={Radio} iconBg="bg-sky-100" iconColor="text-maintenance-cnsd"
        title={`Meter Reading Localizer — ${record.form_number}`}
        subtitle={`${record.form_code} · ${record.location}`}
        actions={
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={() => navigate(`/cnsd/localizer-meter/${record.id}/print`)} className="gap-2">
      ...
      <CnsdLocalizerMeterSignaturePanel record={record} onSignSuccess={handleSignSuccess} />
    </div>
  );
};
