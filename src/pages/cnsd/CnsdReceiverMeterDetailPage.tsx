...
/**
 * CNSD Receiver Meter Reading — detail / edit page.
 *
 ...
 *   - Tabs component, one per section_meta entry (1. RECEIVER, 2. LINGKUNGAN KERJA)
 *   - Section 1 (RECEIVER) renders grouped frequency rows with
 *     STATUS A | STATUS B (ON LINE / OFF LINE dropdowns) | SEQUELSH ON | KETERANGAN
 *   - Section 2 (LINGKUNGAN KERJA) renders KEGIATAN | NOMINAL | HASIL | KETERANGAN
...
export const CnsdReceiverMeterDetailPage: React.FC = () => {
  ...
  const [editedItems, setEditedItems] = useState<Record<number, Partial<CnsdReceiverMeterItem>>>({});
  ...
  // Editable equipment metadata (paper-form header). Flushed on Save alongside item edits.
  ...
  const fetchRecord = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const data = await cnsdReceiverMeterService.getRecord(recordId);
      setRecord(data);
      setEditedItems({});
      setMerk(data.merk ?? '');
      setType(data.type ?? '');
  ...
  const itemsBySection = useMemo(() => {
    const map: Record<string, CnsdReceiverMeterItem[]> = {};
  ...
  const updateField = (
    itemId: number,
    field: keyof CnsdReceiverMeterItem,
    value: string | null,
  ) => {
  ...
  const getValue = (item: CnsdReceiverMeterItem, field: keyof CnsdReceiverMeterItem): string => {
  ...
  const handleSave = async () => {
    ...
    const items = Object.entries(editedItems).map(([rawId, patch]) => ({
      ...
      sequelsh_on: 'sequelsh_on' in patch ? patch.sequelsh_on ?? null : undefined,
  ...
  if (!record) {
    return (
      <div className="max-w-3xl mx-auto py-20 text-center space-y-4">
        ...
        <Button variant="outline" onClick={() => navigate('/cnsd/receiver-meter')} className="gap-2">
          <ArrowLeft size={16} />
          Kembali ke Daftar
        </Button>
      </div>
  ...
  return (
    <div className="max-w-full space-y-5 animate-fade-in pb-12">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/cnsd/receiver-meter/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50">
      ...
      {/* Signature panel */}
      <CnsdReceiverMeterSignaturePanel record={record} onSignSuccess={(r) => setRecord(r)} />
    </div>
  );
};
...
interface ReceiverSectionPanelProps {
  sectionMeta: SectionMeta;
  items: CnsdReceiverMeterItem[];
  isReadOnly: boolean;
  getValue: (item: CnsdReceiverMeterItem, field: keyof CnsdReceiverMeterItem) => string;
  onChange: (itemId: number, field: keyof CnsdReceiverMeterItem, value: string | null) => void;
}

const ReceiverSectionPanel: React.FC<ReceiverSectionPanelProps> = ({
  sectionMeta, items, isReadOnly, getValue, onChange,
}) => {
  // Group items by group_name; Section 1 has header rows we skip in body.
  const groups = useMemo(() => {
    const order: string[] = [];
    const map: Record<string, CnsdReceiverMeterItem[]> = {};
    items.forEach((it) => {
      if (it.is_header) return; // skip explicit header rows; we'll render via group name bar
      ...
      <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between gap-2 bg-slate-50/60">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">
            {isReceiver
              ? 'Pilih status ON LINE / OFF LINE untuk masing-masing channel A dan B per frekuensi.'
              : 'Isi kolom HASIL untuk tiap kegiatan pemeriksaan lingkungan kerja.'}
          </p>
        </div>
        <span className="text-xs font-medium text-slate-400">
          {groups.reduce((sum, g) => sum + g.items.length, 0)} item
        </span>
      </div>
          ...
          <thead>
            <tr className="bg-slate-50 text-slate-700">
              ...
              <th className="px-3 py-2 text-left font-semibold border-b border-slate-200 min-w-[240px] text-[11px] uppercase tracking-wider">
                {isReceiver ? 'Pemeriksaan' : 'Kegiatan'}
              </th>
              {isReceiver ? (
                <>
                  <th className="px-2 py-2 text-center font-semibold border-b border-slate-200 min-w-[120px] text-[11px] uppercase tracking-wider">Status A</th>
                  <th className="px-2 py-2 text-center font-semibold border-b border-slate-200 min-w-[120px] text-[11px] uppercase tracking-wider">Status B</th>
                  <th className="px-2 py-2 text-center font-semibold border-b border-slate-200 min-w-[120px] text-[11px] uppercase tracking-wider">Sequelsh On</th>
                </>
              ) : (
                <>
...
const ReceiverItemRow: React.FC<ReceiverItemRowProps> = ({
  item, rowIdx, isReceiver, isReadOnly, getValue, onChange,
}) => {
        ...
        <>
          <td className="px-2 py-2 align-middle">
            <select
              ...
              onChange={(e) => onChange(item.id, 'status_a', e.target.value || null)}
              disabled={isReadOnly}
            >
              <option value="">—</option>
          ...
          <td className="px-2 py-2 align-middle">
            <select
              ...
              onChange={(e) => onChange(item.id, 'status_b', e.target.value || null)}
              disabled={isReadOnly}
            >
              <option value="">—</option>
              {STATUS_OPTIONS.map((opt) => (
          ...
          <td className="px-2 py-2 align-middle">
            <input
              ...
              placeholder="..."
              value={getValue(item, 'sequelsh_on')}
              onChange={(e) => onChange(item.id, 'sequelsh_on', e.target.value)}
              disabled={isReadOnly}
            />
          </td>
        </>
