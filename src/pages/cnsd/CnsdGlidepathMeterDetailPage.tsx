...
import { CnsdGlidepathMeterSignaturePanel } from './components/CnsdGlidepathMeterSignaturePanel';
...
const SECTION_TABS: { code: SectionTab; label: string }[] = [
  { code: 'A', label: 'Meter Reading' },
  { code: 'B', label: 'Lingkungan Kerja' },
];

// ─── Dropdown helpers ──────────────────────────────────────

function getSingleDropdownOptions(item: CnsdGlidepathMeterItem): string[] | null {
...
export const CnsdGlidepathMeterDetailPage: React.FC = () => {
  ...
  const [localItems, setLocalItems] = useState<CnsdGlidepathMeterItem[]>([]);

  const fetchRecord = useCallback(async () => {
    if (!id) return;
    setIsLoading(true);
    try {
      const data = await cnsdGlidepathMeterService.getRecord(Number(id));
      setRecord(data);
      setLocalItems(data.items);
      setErrorMessage(null);
    } catch {
      setErrorMessage('Gagal memuat data form Glide Path.');
    } finally {
      setIsLoading(false);
    }
  }, [id]);
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await cnsdGlidepathMeterService.updateRecord(record.id, {
  ...
  const handleSignSuccess = (updated: CnsdGlidepathMeterRecordDetail) => {
    setRecord(updated); setLocalItems(updated.items);
  };

  // Grouped items for section A — computed unconditionally (Rules of Hooks)
  const groupedItemsA = useMemo(() => {
    const groups: Map<number, CnsdGlidepathMeterItem[]> = new Map();
  ...
  const sectionBItems = useMemo(
    () => localItems.filter((it) => it.section_code === 'B'),
    [localItems],
  );
  ...
  // ─── Input cell helper ─────────────────────────────────────
  const renderSingleInput = (item: CnsdGlidepathMeterItem) => {
    const opts = getSingleDropdownOptions(item);
    if (isCompleted) {
      return (
        <span className={`text-xs font-medium ${
          item.hasil_1 === 'Normal' || item.hasil_1 === '√' ? 'text-emerald-700'
          : item.hasil_1 === 'Alrm' || item.hasil_1 === '-' || item.hasil_1 === '–' ? 'text-red-600'
          : 'text-slate-700'
        }`}>{item.hasil_1 ?? '—'}</span>
      );
    }
      ...
      return (
        <select value={item.hasil_1 ?? ''} onChange={(e) => handleItemChange(item.id, 'hasil_1', e.target.value || null)}
          className={`w-full h-8 rounded border px-2 text-xs focus:outline-none focus:ring-1 focus:ring-sky-400 ${
            item.hasil_1 === 'Normal' || item.hasil_1 === '√' ? 'bg-emerald-50 border-emerald-200 text-emerald-700'
            : item.hasil_1 === 'Alrm' || item.hasil_1 === '-' || item.hasil_1 === '–' ? 'bg-red-50 border-red-200 text-red-700'
  ...
  const renderDualInput = (item: CnsdGlidepathMeterItem, field: 'hasil_1' | 'hasil_2') => {
    const val = item[field];
    if (isCompleted) return <span className="text-xs text-slate-700">{val ?? '—'}</span>;
    return (
      <input type="text" value={val ?? ''} onChange={(e) => handleItemChange(item.id, field, e.target.value || null)}
        className="w-full h-8 rounded border border-gray-200 px-2 text-xs focus:outline-none focus:ring-1 focus:ring-sky-400"
        placeholder="..." aria-label={`${field === 'hasil_1' ? 'M1' : 'M2'} ${item.item_name}`} />
    );
  };
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <button type="button" onClick={() => navigate('/cnsd/glidepath-meter')} className="hover:text-slate-700 transition-colors">
          Meter Reading Glide Path
        </button>
        <span>/</span>
      ...
      <PageHeader icon={Radio} iconBg="bg-sky-100" iconColor="text-maintenance-cnsd"
        title={`Meter Reading Glide Path — ${record.form_number}`}
        subtitle={`${record.form_code} · ${record.location}`}
        actions={
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={() => navigate(`/cnsd/glidepath-meter/${record.id}/print`)} className="gap-2">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          ...
          {record.serial_number && <div><p className="text-xs text-slate-500">S/N</p><p className="font-medium text-slate-800">{record.serial_number}</p></div>}
            ...
            <table className="w-full text-xs min-w-[700px]">
              <thead>
                <tr className="bg-sky-800 text-white">
                  ...
                  <th className="px-3 py-2 text-center w-28">HASIL / TX1 / M1</th>
                      ...
                      {dataRows.map((item, idx) => {
                        ...
                        return (
                          <tr key={item.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                            ...
                            <td className="px-3 py-2">{renderSingleInput(item)}</td>
                            <td className="px-3 py-2">
                              {isDual ? renderDualInput(item, 'hasil_2') : <span className="text-slate-300 text-xs">—</span>}
                            </td>
                            <td className="px-3 py-2">
                              {isCompleted ? (
      ...
      <CnsdGlidepathMeterSignaturePanel record={record} onSignSuccess={handleSignSuccess} />
    </div>
  );
};
