...
/**
 * CNSD Glide Path Meter Reading — detail / edit page (ILS — GLIDE PATH).
 *
 * Mirrors paper 007_GlidePath. Section A (PEMBACAAN METER READING) has 8 groups
 * with mixed single/dual hasil columns (M1/M2 for CL, DS, CLR, Near Field;
 * single for FRONT PANEL and POWER SUPPLY). Section B (LINGKUNGAN KERJA)
 * uses standard NOMINAL | HASIL | KETERANGAN.
 *
 * UI pattern matches EQ-1 / Radar Meter Reading standard.
 */
export const CnsdGlidepathMeterDetailPage: React.FC = () => {
  ...
  const [editedItems, setEditedItems] = useState<Record<number, Partial<CnsdGlidepathMeterItem>>>({});
  ...
  const fetchRecord = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const data = await cnsdGlidepathMeterService.getRecord(recordId);
      setRecord(data);
      setEditedItems({});
      setMerk(data.merk ?? '');
      setType(data.type ?? '');
  ...
  const itemsBySection = useMemo(() => {
    const map: Record<string, CnsdGlidepathMeterItem[]> = {};
  ...
  const updateField = (itemId: number, field: keyof CnsdGlidepathMeterItem, value: string | null) => {
  ...
  const getValue = (item: CnsdGlidepathMeterItem, field: keyof CnsdGlidepathMeterItem): string => {
  ...
  if (!record) {
    return (
      <div className="max-w-3xl mx-auto py-20 text-center space-y-4">
        ...
        <Button variant="outline" onClick={() => navigate('/cnsd/glidepath-meter')} className="gap-2">
  ...
  return (
    <div className="max-w-full space-y-5 animate-fade-in pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className="h-10 w-10 rounded-lg bg-sky-100 flex items-center justify-center shrink-0">
              <GlidePathIcon size={18} className="text-sky-600" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">Meter Reading — ILS Glide Path</h1>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/cnsd/glidepath-meter/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50">
      ...
      <CnsdGlidepathMeterSignaturePanel record={record} onSignSuccess={(r) => setRecord(r)} />
    </div>
  );
};
...
interface GlidepathSectionPanelProps {
          ...
          <thead>
            <tr className="bg-slate-50 text-slate-700">
              ...
              <th className="px-2 py-2 text-center font-semibold border-b border-slate-200 min-w-[110px] text-[11px] uppercase tracking-wider">
                {isMeterReading ? 'M1 / Hasil' : 'Hasil'}
              </th>
              <th className="px-2 py-2 text-center font-semibold border-b border-slate-200 min-w-[110px] text-[11px] uppercase tracking-wider">M2</th>
