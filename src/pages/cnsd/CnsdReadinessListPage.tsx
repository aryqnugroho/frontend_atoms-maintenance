...
/**
 * Tolerant name comparison — mirrors backend WorkOrderService::namesMatch.
...
/**
 * Determine the sign status of the logged-in user for a summary record.
 * The list API returns `technician_names` (names of ALL technicians on the record)
 * and `manager_name` / `supervisor_name` (always present when assigned).
 *
 * Since signatures are not returned in the list endpoint (we want to avoid
 * fetching detail per row), we check:
 *   - If the record is 'completed', user has no action.
 *   - Otherwise, we check role + name match to decide if user is a relevant signer.
 *   - We cannot know if they've already signed from the summary; badge shows
 *     "Perlu Tanda Tangan" when role+name matches, and we rely on the detail
 *     page to show actual sign status.
 *
 * Returns:
 *   'pending'    — user is a signer and the record is not completed
 *   'completed'  — record is completed
 *   'none'       — user is not a relevant signer
 */
type SignerBadgeStatus = 'pending' | 'completed' | 'none';

const getSignerStatus = (
  record: CnsdReadinessRecordSummary,
  ...
  if (
    (userRole === 'Supervisor CNSD' || userRole === 'Admin') &&
    namesMatch(record.supervisor_name, userName)
  ) {
    return 'pending';
  }

  // Check if user is one of the technicians (name-based since summary has names)
  if (userRole === 'Teknisi CNSD' || userRole === 'Supervisor CNSD') {
    if (record.technician_names.some((n) => namesMatch(n, userName))) {
      return 'pending';
    }
  }
...
/**
 * CNSD Readiness list page (Form EQ-1).
 *
 * Mirrors the Work Order list page pattern: search + filter chips + reset,
 * empty states for "DB kosong" vs "filter kosong", and a single "Buat Kesiapan"
 * button that triggers the create flow.
 *
 * Create flow:
 *   - Sends current shift_date + shift_type computed from the user's local clock
 *     (frontend = source of truth for date/shift; backend runs in UTC).
 *   - On 409 (record already exists), surface the message and navigate to the
 *     existing record so the user doesn't have to search for it.
 */
export const CnsdReadinessListPage: React.FC = () => {
  ...
  // ── data state ─────────────────────────────────────────
  const [records, setRecords] = useState<CnsdReadinessRecordSummary[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await cnsdReadinessService.listRecords(params);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        setErrorMessage('Gagal memuat daftar Kesiapan Peralatan.');
      }
      setRecords([]);
      setTotalCount(0);
    } finally {
  ...
  const handleCreate = async () => {
    ...
    setErrorMessage(null);
    try {
      const record: CnsdReadinessRecordDetail = await cnsdReadinessService.createRecord({
        date: getCurrentShiftDate(),
        shift_type: getCurrentShiftType(),
      });
      navigate(`/cnsd/readiness/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        const data = err.response.data as {
          message?: string;
          errors?: { existing_record?: { id: number; form_number: string } };
        };

        if (err.response.status === 409 && data.errors?.existing_record) {
          // Existing record — navigate to it so the user can continue working
          const id = data.errors.existing_record.id;
          navigate(`/cnsd/readiness/${id}`);
          return;
        }
        ...
        if (err.response.status === 422) {
          setErrorMessage(
            data.message
              ?? 'Tidak ada teknisi CNSD yang bertugas pada shift ini. Pastikan roster sudah dipublish.'
          );
          return;
        }

        setErrorMessage(data.message ?? 'Gagal membuat Form Kesiapan.');
        return;
      }
      setErrorMessage('Gagal membuat Form Kesiapan. Coba lagi.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Hapus form Kesiapan ini? Tindakan ini tidak dapat diurungkan.')) return;
    try {
      await cnsdReadinessService.deleteRecord(id);
      void fetchRecords();
    } catch {
      alert('Gagal menghapus form. Pastikan Anda memiliki izin yang sesuai.');
    }
  };
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Kesiapan Peralatan</span>
      </div>
      ...
      <PageHeader
        ...
        subtitle="Form EQ-1 — Kesiapan peralatan komunikasi, navigasi, pengamatan & automasi penerbangan"
        actions={
          canCreate && (
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 space-y-3">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Cari nomor form, manager, atau supervisor..."
              value={searchQuery}
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            <div className="h-14 w-14 rounded-full bg-sky-50 flex items-center justify-center">
              <CheckSquare size={24} className="text-maintenance-cnsd/70" />
            </div>
            <p className="text-base font-semibold text-slate-700">Belum ada Form Kesiapan</p>
            <p className="text-sm text-slate-400 max-w-md">
              Klik <strong>Buat Kesiapan</strong> untuk membuat Form EQ-1 untuk shift hari ini.
              Backend akan otomatis mengisi teknisi CNSD, supervisor, dan manager teknik dari roster.
            </p>
            {canCreate && (
              <Button onClick={handleCreate} className="gap-2 mt-2" isLoading={isCreating}>
                <Plus size={15} />
                Buat Kesiapan
              </Button>
            )}
            ...
            <table className="w-full text-sm min-w-[800px]">
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  <tr
                    key={r.id}
                    onClick={() => navigate(`/cnsd/readiness/${r.id}`)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' || e.key === ' ') {
                        e.preventDefault();
                        navigate(`/cnsd/readiness/${r.id}`);
                      }
                    }}
                    tabIndex={0}
                    role="button"
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <ShiftBadge shift={r.shift_type} />
                    </td>
                    <td className="px-6 py-4">
                      <div
                        className="inline-flex items-center gap-1 text-slate-700"
                        title={r.technician_names.join(', ')}
                      >
                    ...
                    <td className="px-6 py-4 text-slate-700">
                      {r.supervisor_name ? (
                        <Badge variant="cnsd" className="text-xs">{r.supervisor_name}</Badge>
                      ) : (
                        <span className="text-xs text-slate-400">—</span>
                      )}
                    </td>
                    <td className="px-6 py-4 text-slate-700">
                      {r.manager_name ?? <span className="text-xs text-slate-400">—</span>}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
                        ...
                        <button
                          onClick={() => navigate(`/cnsd/readiness/${r.id}/print`)}
