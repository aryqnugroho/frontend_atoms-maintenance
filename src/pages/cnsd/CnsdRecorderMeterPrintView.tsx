...
interface PrintSectionBlockProps {
  meta: CnsdRecorderMeterSectionMeta;
  items: CnsdRecorderMeterItem[];
}

const PrintSectionBlock: React.FC<PrintSectionBlockProps> = ({ meta, items }) => {
  const isServerDual = meta.inputs_layout === 'server_dual';
  const totalCols = 5;

  if (items.length === 0) {
    return (
      <tr>
        <td colSpan={totalCols} className="gc-section-bar text-center">{meta.code}. {meta.name}</td>
      </tr>
    );
  }
  ...
  return (
    <>
      {/* Section bar */}
      <tr>
        <td className="gc-section-bar text-center" style={{ verticalAlign: 'middle', width: '5%' }}>{meta.code}</td>
        <td colSpan={totalCols - 1} className="gc-section-bar font-bold uppercase">{meta.name}</td>
      </tr>

      {/* Environment section needs its own column sub-header per paper */}
      {!isServerDual && (
        <tr>
          ...
          <td className="gc-group-bar text-center font-bold">HASIL PEMERIKSAAN</td>
            ...
            if (item.is_blocked) {
              return (
                <tr key={item.id} className="text-[10px]">
                  ...
                  <td colSpan={3} className="text-center font-bold uppercase" style={{ backgroundColor: '#ef4444', color: '#fff' }}>
...
/**
 * CNSD Recorder Meter Reading Print View — A4 portrait, B&W.
 *
 * Layout mirrors the official "METER READING — RECORDER" paper form
 * 003_Recorder (FORM C-3):
 *   - Kop band: AirNav logo + "FAS CNS & A" (left), Perum LPPNPI address (right)
 *   - Title METER READING centered, RECORDER green bar in middle column
 *   - Metadata band: LOKASI/TGL JAM (left) · RECORDER (center) · MERK/TYPE/SN (right)
 *   - Section A (PERALATAN) — 5 cols: NO | PEMBACAAN | NOMINAL | SERVER A | SERVER B
 *     with sub-groups KVM / SERVER / POWER / CHANNEL. U/S channels render as
 *     red strip with "U/S" label.
 *   - Section B (LINGKUNGAN KERJA) — 5 cols: NO | KEGIATAN | NOMINAL | HASIL | KETERANGAN
 *   - Footer 3-col: TEKNISI sub-table | SUPERVISOR | MANAGER TEKNIK
 *
 * Per project standard: NO form_number on print. Grayscale only (U/S red strip
 * is preserved to mark unavailable channels per paper form convention).
 */
  ...
  if (!record) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4">
        ...
        <Button variant="outline" onClick={() => navigate('/cnsd/recorder-meter')}>Kembali</Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full bg-slate-100 p-4 text-black print:bg-white print:p-0">
      ...
      <div className="print-hide mx-auto mb-4 flex max-w-[210mm] items-center justify-between">
        <Button variant="outline" className="gap-2" onClick={() => navigate(`/cnsd/recorder-meter/${record.id}`)}>
        ...
        <div className="flex items-start justify-between gap-3 mb-2">
          <div className="flex items-center gap-2">
            ...
            <div className="leading-tight">
              ...
              <div className="text-[8px] font-semibold uppercase">FAS CNS &amp; A</div>
            </div>
          </div>
        ...
        <table className="gc-table mb-0">
          ...
          <tbody>
            <tr>
              ...
              <td className="text-right font-bold text-[10px]">{text(record.form_code) || 'FORM C-3'}</td>
            </tr>
            <tr>
                ...
                <table className="gc-meta text-[10px]">
                  <tbody>
                    <tr>
                      ...
                      <td className="px-0.5">:</td>
                      <td>{text(record.merk) || 'ATIS - UHER'}</td>
                    </tr>
                    <tr>
                      <td className="font-semibold pr-1">TYPE</td>
                      <td className="px-0.5">:</td>
                      <td>{text(record.type) || 'VC - MDx'}</td>
                    </tr>
                    <tr>
                      <td className="font-semibold pr-1">S N</td>
                      <td className="px-0.5">:</td>
                      <td>{text(record.serial_number) || '51'}</td>
                    </tr>
                  </tbody>
                </table>
              </td>
        ...
        <table className="gc-table">
          ...
          <thead>
            <tr className="text-center font-bold gc-section-bar">
              ...
              <th className="border border-black py-1" rowSpan={2}>PEMBACAAN METER READING</th>
              <th className="border border-black py-1" rowSpan={2}>NOMINAL</th>
              <th colSpan={2} className="border border-black py-1">HASIL PEMERIKSAAN</th>
            </tr>
            <tr className="text-center font-bold gc-section-bar">
              <th className="border border-black py-1">Server A</th>
              <th className="border border-black py-1">Server B</th>
            </tr>
          </thead>
          <tbody>
