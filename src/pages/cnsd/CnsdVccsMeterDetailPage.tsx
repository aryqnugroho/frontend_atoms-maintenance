 ...
 * render for the FRONT PANEL adaptive cell. Mirrors the same parsing strategy
 * used by ATC SYSTEM's AdaptiveReadingCell.
 *
 * Rules (uppercase, trimmed):
...
/**
 * CNSD VCCS LES Meter Reading — detail / edit page.
 *
 * VCCS has 3 distinct layouts handled per-section:
 *   1. dual_adaptive  → FRONT PANEL (Server A | Server B), adapts per nominal
 *   2. dual_toggle_nf → MSC & RCMS / CWP, two √/- toggles labeled Normal | Fault
 *   3. environment    → LINGKUNGAN KERJA (single result column)
 *
 * UI follows EQ-1 / Radar / AMSC template-lock:
 *   - Header card with status pill + date / shift / facility / teknisi badges
 *   - Info Peralatan card with editable Merk / Type / SN (Manager/Supervisor)
 *   - Tabs per section (including the 3 sub-sections of A. PERALATAN)
 *   - Signature panel at the bottom
 */
export const CnsdVccsMeterDetailPage: React.FC = () => {
  ...
  const [editedItems, setEditedItems] = useState<Record<number, Partial<CnsdVccsMeterItem>>>({});
  ...
  const getValue = (item: CnsdVccsMeterItem, field: keyof CnsdVccsMeterItem): string => {
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await cnsdVccsMeterService.updateRecord(record.id, {
    ...
    <div className="max-w-full space-y-5 animate-fade-in pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className="h-10 w-10 rounded-lg bg-sky-100 flex items-center justify-center shrink-0">
              <VccsIcon size={18} className="text-sky-600" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">Meter Reading — VCCS</h1>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/cnsd/vccs-meter/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs pt-2 border-t border-slate-100">
          <EditableMetaField label="Merk" value={merk} onChange={setMerk} disabled={isReadOnly || !canEditMetadata} placeholder="LES" />
      ...
      <CnsdVccsMeterSignaturePanel record={record} onSignSuccess={() => void fetchRecord()} />
    </div>
  );
};
...
interface VccsSectionPanelProps {
  sectionMeta: CnsdVccsMeterSectionMeta;
  items: CnsdVccsMeterItem[];
  isReadOnly: boolean;
  getValue: (item: CnsdVccsMeterItem, field: keyof CnsdVccsMeterItem) => string;
...
const VccsSectionPanel: React.FC<VccsSectionPanelProps> = ({
  ...
  // Group items by group_name for sections that have sub-groups (e.g. MSC & RCMS: Frame 1 / Frame 2)
  ...
  // Whether to render group bars between sub-groups. Only show if section
  // has >1 distinct named groups (skip when the section is one big block).
...
interface VccsItemRowProps {
  item: CnsdVccsMeterItem;
  layout: CnsdVccsInputsLayout;
  isReadOnly: boolean;
  getValue: (item: CnsdVccsMeterItem, field: keyof CnsdVccsMeterItem) => string;
  onChange: (itemId: number, field: keyof CnsdVccsMeterItem, value: string | null) => void;
}
...
// ─── Adaptive cell (FRONT PANEL Server A/B) ───────────────────
...
/**
 * MSC & RCMS and CWP sections want a single √-or-blank toggle per cell.
 * Clicking once sets √, clicking again clears it. Color coded:
 *   - Normal column → emerald when checked
...
// ─── Environment cell (LINGKUNGAN KERJA) ──────────────────────
...
/**
 * For environment items where the nominal is just "√", render a single toggle.
 * For numeric nominals (e.g. "< 22°C"), render a free-text input so the user
