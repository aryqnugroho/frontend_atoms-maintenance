...
import type { CnsdReceiverMeterRecordSummary, CnsdReceiverMeterRecordDetail } from '@/types/cnsdReceiver';
...
const getSignerStatus = (
  record: CnsdReceiverMeterRecordSummary,
...
export const CnsdReceiverMeterListPage: React.FC = () => {
  ...
  const [records, setRecords] = useState<CnsdReceiverMeterRecordSummary[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await cnsdReceiverMeterService.listRecords(params);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        setErrorMessage('Gagal memuat daftar Meter Reading Receiver.');
      }
      setRecords([]);
      setTotalCount(0);
    } finally {
  ...
  const handleCreate = async () => {
    ...
    setErrorMessage(null);
    try {
      const record: CnsdReceiverMeterRecordDetail = await cnsdReceiverMeterService.createRecord({
        date: getCurrentShiftDate(),
        shift_type: getCurrentShiftType(),
      });
      navigate(`/cnsd/receiver-meter/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        const data = err.response.data as {
          message?: string;
          errors?: { existing_record?: { id: number } };
        };
        if (err.response.status === 409 && data.errors?.existing_record) {
          navigate(`/cnsd/receiver-meter/${data.errors.existing_record.id}`);
          return;
        }
        ...
        setErrorMessage(data.message ?? 'Gagal membuat Meter Reading Receiver.');
        return;
      }
      setErrorMessage('Gagal membuat Meter Reading Receiver. Coba lagi.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Hapus form Meter Reading Receiver ini?')) return;
    try {
      await cnsdReceiverMeterService.deleteRecord(id);
      void fetchRecords();
    } catch {
  ...
  if (debouncedSearch)
    activeFilters.push({ key: 'search', label: `"${debouncedSearch}"`, clear: () => { setSearchQuery(''); setDebouncedSearch(''); } });
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Meter Reading Receiver</span>
      </div>
      ...
      <PageHeader
        ...
        title="Meter Reading Receiver"
        subtitle="CNSD Equipment Readiness — Receiver (FORM C-2)"
        actions={
          canCreate && (
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 space-y-3">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Cari nomor form, manager, supervisor..."
              value={searchQuery}
              ...
              aria-label="Cari Meter Reading Receiver"
            />
          </div>
          {hasActiveFilters && (
            <button
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            <div className="h-14 w-14 rounded-full bg-sky-50 flex items-center justify-center">
              <Radio size={24} className="text-maintenance-cnsd/70" />
            </div>
            <p className="text-base font-semibold text-slate-700">Belum ada Meter Reading Receiver</p>
            <p className="text-sm text-slate-400 max-w-md">
              Klik <strong>Buat Meter Reading</strong> untuk membuat form Receiver baru untuk shift hari ini.
            </p>
            {canCreate && (
              <Button onClick={handleCreate} className="gap-2 mt-2" isLoading={isCreating}>
                <Plus size={15} /> Buat Meter Reading
              </Button>
            )}
          </div>
        ) : isFilterEmpty ? (
            ...
            <table className="w-full text-sm min-w-[800px]">
              <thead>
                <tr className="border-b border-gray-100">
                  {['No. Form', 'Tanggal', 'Shift', 'Teknisi', 'Supervisor', 'Manager', 'Status', 'Anda', 'Aksi'].map(
                    (h) => (
                      <th
                        key={h}
                        className="text-left text-xs font-medium text-slate-500 uppercase tracking-wider px-6 py-3 bg-gray-50/80"
                      >
                        {h}
                      </th>
                    ),
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr
                      key={r.id}
                      onClick={() => navigate(`/cnsd/receiver-meter/${r.id}`)}
                      tabIndex={0}
                      role="button"
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <ShiftBadge shift={r.shift_type as import('@/types').ShiftType} />
                      </td>
                      <td className="px-6 py-4">
                        <div
                      ...
                      <td className="px-6 py-4 text-slate-700">
                        {r.supervisor_name ? (
                          <Badge variant="cnsd" className="text-xs">
                            {r.supervisor_name}
                          </Badge>
                        ) : (
                      ...
                      <td className="px-6 py-4 text-center">
                        ...
                        >
                          ...
                          <button
                            onClick={() => navigate(`/cnsd/receiver-meter/${r.id}/print`)}
