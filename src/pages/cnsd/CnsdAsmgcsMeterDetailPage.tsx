...
/**
 * Adaptive shape parser for FRONT PANEL single_adaptive cell, mirrors VCCS Freq:
...
/**
 * CNSD ASMGCS (SAAB) Meter Reading — detail / edit page.
 * Same layout pattern as VCCS Freq (CNSD-015) minus the CWP section.
 */
export const CnsdAsmgcsMeterDetailPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { user } = useAuth();
  const recordId = Number(id);

  const [record, setRecord] = useState<CnsdAsmgcsMeterRecordDetail | null>(null);
  const [editedItems, setEditedItems] = useState<Record<number, Partial<CnsdAsmgcsMeterItem>>>({});
  ...
  const fetchRecord = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const data = await cnsdAsmgcsMeterService.getRecord(recordId);
      setRecord(data);
      setEditedItems({});
      setMerk(data.merk ?? '');
      setType(data.type ?? '');
  ...
  const itemsBySection = useMemo(() => {
    const map: Record<string, CnsdAsmgcsMeterItem[]> = {};
  ...
  const updateField = (itemId: number, field: keyof CnsdAsmgcsMeterItem, value: string | null) => {
  ...
  const getValue = (item: CnsdAsmgcsMeterItem, field: keyof CnsdAsmgcsMeterItem): string => {
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await cnsdAsmgcsMeterService.updateRecord(record.id, {
  ...
  if (!record) {
    return (
      <div className="max-w-3xl mx-auto py-20 text-center space-y-4">
        ...
        <Button variant="outline" onClick={() => navigate('/cnsd/asmgcs-meter')} className="gap-2">
          <ArrowLeft size={16} />
          Kembali ke Daftar
        </Button>
      </div>
  ...
  return (
    <div className="max-w-full space-y-5 animate-fade-in pb-12">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <button type="button" onClick={() => navigate('/cnsd/asmgcs-meter')} className="hover:text-slate-700 transition-colors">
          ASMGCS Meter Reading
        </button>
        <span>/</span>
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className="h-10 w-10 rounded-lg bg-sky-100 flex items-center justify-center shrink-0">
              <AsmgcsIcon size={18} className="text-sky-600" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">Meter Reading — ASMGCS</h1>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/cnsd/asmgcs-meter/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs pt-2 border-t border-slate-100">
          <EditableMetaField label="Merk" value={merk} onChange={setMerk} disabled={isReadOnly || !canEditMetadata} placeholder="SAAB" />
      ...
      <CnsdAsmgcsMeterSignaturePanel record={record} onSignSuccess={() => void fetchRecord()} />
    </div>
  );
};
...
interface AsmgcsSectionPanelProps {
  sectionMeta: CnsdAsmgcsMeterSectionMeta;
  items: CnsdAsmgcsMeterItem[];
  isReadOnly: boolean;
  getValue: (item: CnsdAsmgcsMeterItem, field: keyof CnsdAsmgcsMeterItem) => string;
  onChange: (itemId: number, field: keyof CnsdAsmgcsMeterItem, value: string | null) => void;
}

const AsmgcsSectionPanel: React.FC<AsmgcsSectionPanelProps> = ({
  sectionMeta, items, isReadOnly, getValue, onChange,
}) => {
  const layout: CnsdAsmgcsInputsLayout = sectionMeta.inputs_layout;
  ...
  const description = (() => {
    switch (layout) {
      case 'single_adaptive': return 'Isi Redundant Server — toggle untuk Normal/Alarm dan √/-, input bebas untuk lainnya.';
  ...
  const groupedItems = useMemo(() => {
    const groups: { name: string | null; items: CnsdAsmgcsMeterItem[] }[] = [];
    let current: { name: string | null; items: CnsdAsmgcsMeterItem[] } | null = null;
  ...
  // Section 2 = CENTRAL SERVER uses "Status" as the 3rd column header (paper),
  // others use "Standart" (single_adaptive) or "Nominal" (environment).
  const col3Header = isEnv
    ? 'Nominal'
    : (isSingleAdaptive ? 'Standart' : 'Status');
...
interface AsmgcsItemRowProps {
  item: CnsdAsmgcsMeterItem;
  layout: CnsdAsmgcsInputsLayout;
  isReadOnly: boolean;
  getValue: (item: CnsdAsmgcsMeterItem, field: keyof CnsdAsmgcsMeterItem) => string;
  onChange: (itemId: number, field: keyof CnsdAsmgcsMeterItem, value: string | null) => void;
}

const AsmgcsItemRow: React.FC<AsmgcsItemRowProps> = ({
...
// ─── Single-check toggle cell (CENTRAL SERVER) ────────────────
