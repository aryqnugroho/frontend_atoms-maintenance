...
const PrintBlock: React.FC<PrintBlockProps> = ({ meta, items }) => {
  ...
  // Group by group_number+name (skip headers)
  const groups: { number: number | null; name: string | null; items: CnsdGlidepathMeterItem[] }[] = [];
  items.forEach((it) => {
      ...
      {groups.map((group, gIdx) => (
        <React.Fragment key={`${meta.code}-grp-${gIdx}`}>
          <tr>
            ...
            {group.items.some((i) => i.hasil_layout === 'dual') ? (
              <>
                <td className="gc-group-bar text-center font-bold">M1</td>
                <td className="gc-group-bar text-center font-bold">M2</td>
              </>
            ) : (
...
/**
 * CNSD Glide Path Meter Reading Print View — A4 portrait, B&W.
 * Mirrors paper 007_GlidePath.
 */
  ...
  if (!record) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4">
        ...
        <Button variant="outline" onClick={() => navigate('/cnsd/glidepath-meter')}>Kembali</Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full bg-slate-100 p-4 text-black print:bg-white print:p-0">
      ...
      <div className="print-hide mx-auto mb-4 flex max-w-[210mm] items-center justify-between">
        <Button variant="outline" className="gap-2" onClick={() => navigate(`/cnsd/glidepath-meter/${record.id}`)}>
