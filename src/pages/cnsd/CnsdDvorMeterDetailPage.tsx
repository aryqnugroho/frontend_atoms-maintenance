...
export const CnsdDvorMeterDetailPage: React.FC = () => {
  ...
  const [editedItems, setEditedItems] = useState<Record<number, Partial<CnsdDvorMeterItem>>>({});
  ...
  const getValue = (item: CnsdDvorMeterItem, field: keyof CnsdDvorMeterItem): string => {
    ...
    <div className="max-w-full space-y-5 animate-fade-in pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className="h-10 w-10 rounded-lg bg-sky-100 flex items-center justify-center shrink-0">
              <DvorIcon size={18} className="text-sky-600" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg font-bold text-slate-900">Meter Reading — DVOR</h1>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigate(`/cnsd/dvor-meter/${record.id}/print`)} className="gap-1.5 text-indigo-600 hover:bg-indigo-50">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs pt-2 border-t border-slate-100">
          ...
          <EditableMetaField label="Type" value={type} onChange={setType} disabled={!!isReadOnly || !canEditMetadata} placeholder="VRB 52 D" />
      ...
      <CnsdDvorMeterSignaturePanel record={record} onSignSuccess={(r) => setRecord(r)} />
    </div>
  );
};

interface DvorSectionPanelProps {
