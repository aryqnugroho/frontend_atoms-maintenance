...
import type { CnsdAmscMeterRecordDetail, CnsdAmscMeterRecordSummary } from '@/types/cnsdAmsc';
...
const getSignerStatus = (
  record: CnsdAmscMeterRecordSummary,
...
export const CnsdAmscMeterListPage: React.FC = () => {
  ...
  const [records, setRecords] = useState<CnsdAmscMeterRecordSummary[]>([]);
  ...
  useEffect(() => {
    cnsdAmscMeterService.getYears().then(setAvailableYears).catch(() => setAvailableYears([new Date().getFullYear()]));
  }, []);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await cnsdAmscMeterService.listRecords(params);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        setErrorMessage('Gagal memuat daftar Meter Reading AMSC.');
      }
      setRecords([]);
      setTotalCount(0);
    } finally {
  ...
  const handleCreate = async () => {
    ...
    setErrorMessage(null);
    try {
      const record: CnsdAmscMeterRecordDetail = await cnsdAmscMeterService.createRecord({
        date: getCurrentShiftDate(),
        shift_type: getCurrentShiftType(),
      });
      navigate(`/cnsd/amsc-meter/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        ...
        if (err.response.status === 409 && data.errors?.existing_record) {
          navigate(`/cnsd/amsc-meter/${data.errors.existing_record.id}`);
          return;
        }
        ...
        setErrorMessage(data.message ?? 'Gagal membuat Meter Reading AMSC.');
        return;
      }
      setErrorMessage('Gagal membuat Meter Reading AMSC. Coba lagi.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Hapus form Meter Reading AMSC ini?')) return;
    try { await cnsdAmscMeterService.deleteRecord(id); void fetchRecords(); } catch { alert('Gagal menghapus form.'); }
  };
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Meter Reading AMSC</span>
      </div>
      ...
      <PageHeader
        ...
        subtitle="CNSD Equipment Readiness — AMSC (ELSA 1003Qi+)"
        actions={canCreate && (
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            <div className="h-14 w-14 rounded-full bg-sky-50 flex items-center justify-center"><AmscIcon size={24} className="text-maintenance-cnsd/70" /></div>
            <p className="text-base font-semibold text-slate-700">Belum ada Meter Reading AMSC</p>
            <p className="text-sm text-slate-400 max-w-md">Klik <strong>Buat Meter Reading</strong> untuk membuat form AMSC baru untuk shift hari ini.</p>
            ...
            <table className="w-full text-sm min-w-[800px]">
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr key={r.id} onClick={() => navigate(`/cnsd/amsc-meter/${r.id}`)} tabIndex={0} role="button" className="border-b border-gray-50 hover:bg-gray-50/50 cursor-pointer transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-primary">
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => navigate(`/cnsd/amsc-meter/${r.id}`)} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Detail / Edit"><Edit2 size={16} /></button>
                          <button onClick={() => navigate(`/cnsd/amsc-meter/${r.id}/print`)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Print PDF"><Printer size={16} /></button>
