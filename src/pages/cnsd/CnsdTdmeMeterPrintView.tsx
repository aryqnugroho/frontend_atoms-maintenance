...
const PrintBlock: React.FC<PrintBlockProps> = ({ meta, items }) => {
  ...
  const groups: { number: number | null; name: string | null; items: CnsdTdmeMeterItem[] }[] = [];
  items.forEach((it) => {
...
/**
 * CNSD T-DME Meter Reading Print View — A4 portrait, B&W. Mirrors paper 009_T-DME (FORM N-5).
 */
export const CnsdTdmeMeterPrintView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRecord = async () => {
      try { setRecord(await cnsdTdmeMeterService.getRecord(Number(id))); }
  ...
  if (!record) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4">
        ...
        <Button variant="outline" onClick={() => navigate('/cnsd/tdme-meter')}>Kembali</Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full bg-slate-100 p-4 text-black print:bg-white print:p-0">
      ...
      <div className="print-hide mx-auto mb-4 flex max-w-[210mm] items-center justify-between">
        <Button variant="outline" className="gap-2" onClick={() => navigate(`/cnsd/tdme-meter/${record.id}`)}>
        ...
        <table className="gc-table mb-0">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td className="gc-section-bar text-center font-bold uppercase text-[12px]">T-DME</td>
              <td className="align-top leading-tight">
