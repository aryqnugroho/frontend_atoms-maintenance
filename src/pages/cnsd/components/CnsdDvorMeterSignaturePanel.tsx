...
import type { CnsdDvorMeterRecordDetail, CnsdDvorMeterTechnician, CnsdDvorMeterSignerInfo } from '@/types/cnsdDvor';

interface Props {
  record: CnsdDvorMeterRecordDetail;
  onSignSuccess: (record: CnsdDvorMeterRecordDetail) => void;
}
...
type Column =
  | { kind: 'manager' | 'supervisor'; label: string; info: CnsdDvorMeterSignerInfo | null }
  | { kind: 'technician'; label: string; row: CnsdDvorMeterTechnician };

export const CnsdDvorMeterSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const updated = await cnsdDvorMeterService.signRecord(record.id, { role: pending.role, signature, technician_row_id: pending.technicianRowId });
