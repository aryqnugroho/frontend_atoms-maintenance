...
import type { CnsdAtisMeterRecordDetail, CnsdAtisMeterTechnician, CnsdAtisMeterSignerInfo } from '@/types/cnsdAtis';

interface Props {
  record: CnsdAtisMeterRecordDetail;
  onSignSuccess: (record: CnsdAtisMeterRecordDetail) => void;
}
...
type Column =
  | { kind: 'manager' | 'supervisor'; label: string; info: CnsdAtisMeterSignerInfo | null }
  | { kind: 'technician'; label: string; row: CnsdAtisMeterTechnician };

export const CnsdAtisMeterSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const updated = await cnsdAtisMeterService.signRecord(record.id, { role: pending.role, signature, technician_row_id: pending.technicianRowId });
