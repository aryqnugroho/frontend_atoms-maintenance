...
import { cnsdTransmitterMeterService, type CnsdTransmitterMeterRoleKey } from '@/services/cnsdTransmitterMeterService';
import type {
  CnsdTransmitterMeterRecordDetail,
  CnsdTransmitterMeterTechnician,
} from '@/types/cnsdTransmitter';

interface Props {
  record: CnsdTransmitterMeterRecordDetail;
  onRecordUpdate: (record: CnsdTransmitterMeterRecordDetail) => void;
}
...
export const CnsdTransmitterMeterSignaturePanel: React.FC<Props> = ({ record, onRecordUpdate }) => {
  const { user } = useAuth();
  ...
  type Column =
    | { kind: 'manager' | 'supervisor'; label: string; info: SignerInfo | null }
    | { kind: 'technician'; label: string; row: CnsdTransmitterMeterTechnician };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await cnsdTransmitterMeterService.signRecord(record.id, pending.role, signature, pending.technicianRowId);
