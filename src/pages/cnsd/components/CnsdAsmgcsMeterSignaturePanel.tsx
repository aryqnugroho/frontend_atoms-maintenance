...
import type {
  ...
  CnsdAsmgcsMeterSignerInfo,
  CnsdAsmgcsMeterTechnicianRow,
} from '@/types/cnsdAsmgcs';

interface Props {
  record: CnsdAsmgcsMeterRecordDetail;
  onSignSuccess: () => void;
}
...
export const CnsdAsmgcsMeterSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  type Column =
    | { kind: 'manager' | 'supervisor'; label: string; info: CnsdAsmgcsMeterSignerInfo | null }
    | { kind: 'technician'; label: string; row: CnsdAsmgcsMeterTechnicianRow };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      await cnsdAsmgcsMeterService.signRecord(record.id, pending.role, signature, pending.technicianRowId);
      onSignSuccess();
    } catch (err) {
