...
import type {
  ...
  CnsdReadinessSignerInfo,
  CnsdReadinessTechnicianRow,
} from '@/types/cnsd';

interface Props {
  record: CnsdReadinessRecordDetail;
  onUpdated: (record: CnsdReadinessRecordDetail) => void;
}
...
/**
 * CNSD Readiness signature panel.
 *
 * Renders three blocks:
 *   1. Manager Teknik (single signature row on the record)
 *   2. Supervisor CNSD (single signature row on the record)
 *   3. Teknisi CNSD — one row per technician on duty for this shift
 *
 * Sign buttons are only enabled when:
 *   - The record is not yet completed.
 *   - The current user's name (tolerant compare) matches the cached signer name.
 *   - That role has not been signed yet (immutable).
 *
 ...
 */
export const CnsdReadinessSignaturePanel: React.FC<Props> = ({ record, onUpdated }) => {
  const { user } = useAuth();
  ...
  // ── Build the signer column blocks ───────────────────
  type Column =
    | {
        kind: 'manager' | 'supervisor';
        label: string;
        info: CnsdReadinessSignerInfo | null;
      }
    | {
        kind: 'technician';
        label: string;
        row: CnsdReadinessTechnicianRow;
      };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await cnsdReadinessService.signRecord(
        record.id,
        pending.role,
        signature,
