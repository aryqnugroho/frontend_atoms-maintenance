...
import type {
  ...
  CnsdVccsMeterSignerInfo,
  CnsdVccsMeterTechnicianRow,
} from '@/types/cnsdVccs';

interface Props {
  record: CnsdVccsMeterRecordDetail;
  onSignSuccess: () => void;
}
...
export const CnsdVccsMeterSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  type Column =
    | { kind: 'manager' | 'supervisor'; label: string; info: CnsdVccsMeterSignerInfo | null }
    | { kind: 'technician'; label: string; row: CnsdVccsMeterTechnicianRow };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      await cnsdVccsMeterService.signRecord(record.id, pending.role, signature, pending.technicianRowId);
      onSignSuccess();
    } catch (err) {
