...
import type { CnsdAtcSystemMeterRecordDetail, CnsdAtcSystemMeterTechnician, CnsdAtcSystemMeterSignerInfo } from '@/types/cnsdAtcSystem';

interface Props {
  record: CnsdAtcSystemMeterRecordDetail;
  onSignSuccess: (record: CnsdAtcSystemMeterRecordDetail) => void;
}
...
type Column =
  | { kind: 'manager' | 'supervisor'; label: string; info: CnsdAtcSystemMeterSignerInfo | null }
  | { kind: 'technician'; label: string; row: CnsdAtcSystemMeterTechnician };

export const CnsdAtcSystemMeterSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const updated = await cnsdAtcSystemMeterService.signRecord(record.id, { role: pending.role, signature, technician_row_id: pending.technicianRowId });
