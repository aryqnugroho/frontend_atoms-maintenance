...
import type { CnsdTdmeMeterRecordDetail, CnsdTdmeMeterTechnician, CnsdTdmeMeterSignerInfo } from '@/types/cnsdTdme';

interface Props {
  record: CnsdTdmeMeterRecordDetail;
  onSignSuccess: (record: CnsdTdmeMeterRecordDetail) => void;
}
...
type Column =
  | { kind: 'manager' | 'supervisor'; label: string; info: CnsdTdmeMeterSignerInfo | null }
  | { kind: 'technician'; label: string; row: CnsdTdmeMeterTechnician };

export const CnsdTdmeMeterSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const updated = await cnsdTdmeMeterService.signRecord(record.id, { role: pending.role, signature, technician_row_id: pending.technicianRowId });
