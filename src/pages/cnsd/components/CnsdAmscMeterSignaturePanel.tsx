...
import type {
  ...
  CnsdAmscMeterSignerInfo,
  CnsdAmscMeterTechnicianRow,
} from '@/types/cnsdAmsc';

interface Props {
  record: CnsdAmscMeterRecordDetail;
  onSignSuccess: () => void;
}
...
export const CnsdAmscMeterSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  type Column =
    | { kind: 'manager' | 'supervisor'; label: string; info: CnsdAmscMeterSignerInfo | null }
    | { kind: 'technician'; label: string; row: CnsdAmscMeterTechnicianRow };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      await cnsdAmscMeterService.signRecord(record.id, pending.role, signature, pending.technicianRowId);
      onSignSuccess();
    } catch (err) {
