...
import type { CnsdDmeMeterRecordDetail, CnsdDmeMeterTechnician, CnsdDmeMeterSignerInfo } from '@/types/cnsdDme';

interface Props {
  record: CnsdDmeMeterRecordDetail;
  onSignSuccess: (record: CnsdDmeMeterRecordDetail) => void;
}
...
type Column =
  | { kind: 'manager' | 'supervisor'; label: string; info: CnsdDmeMeterSignerInfo | null }
  | { kind: 'technician'; label: string; row: CnsdDmeMeterTechnician };

export const CnsdDmeMeterSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const updated = await cnsdDmeMeterService.signRecord(record.id, { role: pending.role, signature, technician_row_id: pending.technicianRowId });
