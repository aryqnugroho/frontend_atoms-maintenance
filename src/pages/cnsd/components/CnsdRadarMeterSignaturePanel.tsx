...
import type {
  ...
  CnsdRadarMeterSignerInfo,
  CnsdRadarMeterTechnicianRow,
} from '@/types/cnsdRadar';

interface Props {
  record: CnsdRadarMeterRecordDetail;
  onUpdated: (record: CnsdRadarMeterRecordDetail) => void;
}
...
/**
 * CNSD Radar Meter signature panel.
 *
 * Identical UX/contract to CnsdReadinessSignaturePanel. Renders three blocks:
 ...
 */
export const CnsdRadarMeterSignaturePanel: React.FC<Props> = ({ record, onUpdated }) => {
  const { user } = useAuth();
  ...
  type Column =
    | {
        ...
        label: string;
        info: CnsdRadarMeterSignerInfo | null;
      }
    | {
        kind: 'technician';
        label: string;
        row: CnsdRadarMeterTechnicianRow;
      };
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const result = await cnsdRadarMeterService.signRecord(
        record.id,
        pending.role,
        signature,
