...
import type { CnsdGlidepathMeterRecordDetail, CnsdGlidepathMeterTechnician, CnsdGlidepathMeterSignerInfo } from '@/types/cnsdGlidepath';

interface Props {
  record: CnsdGlidepathMeterRecordDetail;
  onSignSuccess: (record: CnsdGlidepathMeterRecordDetail) => void;
}
...
type Column =
  | { kind: 'manager' | 'supervisor'; label: string; info: CnsdGlidepathMeterSignerInfo | null }
  | { kind: 'technician'; label: string; row: CnsdGlidepathMeterTechnician };

export const CnsdGlidepathMeterSignaturePanel: React.FC<Props> = ({ record, onSignSuccess }) => {
  const { user } = useAuth();
  ...
  const handleSign = async (signature: string) => {
    ...
    try {
      const updated = await cnsdGlidepathMeterService.signRecord(record.id, { role: pending.role, signature, technician_row_id: pending.technicianRowId });
