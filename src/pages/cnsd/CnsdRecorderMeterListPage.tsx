...
const getSignerStatus = (
  record: CnsdRecorderMeterRecordSummary,
  ...
  if ((userRole === 'Supervisor CNSD' || userRole === 'Admin') && namesMatch(record.supervisor_name, userName)) {
    return 'pending';
  }
...
/**
 * CNSD Recorder Meter Reading list page.
 *
 * Mirrors the Radar Meter list pattern: search + filter chips + reset,
 * empty-state DB-empty vs filter-empty, signer status badge, single
 * "Buat Meter Reading" button.
 */
export const CnsdRecorderMeterListPage: React.FC = () => {
  ...
  // ── data state ─────────────────────────────────────────
  const [records, setRecords] = useState<CnsdRecorderMeterRecordSummary[]>([]);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await cnsdRecorderMeterService.listRecords(params);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        setErrorMessage('Gagal memuat daftar Meter Reading Recorder.');
      }
      setRecords([]);
      setTotalCount(0);
    } finally {
  ...
  const handleCreate = async () => {
    ...
    setErrorMessage(null);
    try {
      const record: CnsdRecorderMeterRecordDetail = await cnsdRecorderMeterService.createRecord({
        date: getCurrentShiftDate(),
        shift_type: getCurrentShiftType(),
      });
      navigate(`/cnsd/recorder-meter/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        ...
        if (err.response.status === 409 && data.errors?.existing_record) {
          ...
          navigate(`/cnsd/recorder-meter/${id}`);
          return;
        }
        ...
        setErrorMessage(data.message ?? 'Gagal membuat Meter Reading Recorder.');
        return;
      }
      setErrorMessage('Gagal membuat Meter Reading Recorder. Coba lagi.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Hapus form Meter Reading Recorder ini? Tindakan ini tidak dapat diurungkan.')) return;
    try {
      await cnsdRecorderMeterService.deleteRecord(id);
      void fetchRecords();
    } catch {
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Meter Reading Recorder</span>
      </div>
      ...
      <PageHeader
        ...
        title="Meter Reading Recorder"
        subtitle="CNSD Equipment Readiness — Recorder (ATIS - UHER VC - MDx, FORM C-3)"
        actions={
          canCreate && (
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 space-y-3">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Cari nomor form, manager, supervisor, atau SN..."
              value={searchQuery}
              ...
              aria-label="Cari Meter Reading Recorder"
            />
          </div>
          {hasActiveFilters && (
            <button
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            <div className="h-14 w-14 rounded-full bg-sky-50 flex items-center justify-center">
              <RecorderIcon size={24} className="text-maintenance-cnsd/70" />
            </div>
            <p className="text-base font-semibold text-slate-700">Belum ada Meter Reading Recorder</p>
            <p className="text-sm text-slate-400 max-w-md">
              Klik <strong>Buat Meter Reading</strong> untuk membuat form Recorder baru untuk shift hari ini.
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr
                      key={r.id}
                      onClick={() => navigate(`/cnsd/recorder-meter/${r.id}`)}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault();
                          navigate(`/cnsd/recorder-meter/${r.id}`);
                        }
                      }}
                      tabIndex={0}
                      role="button"
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
                          ...
                          <button
                            onClick={() => navigate(`/cnsd/recorder-meter/${r.id}/print`)}
