...
import type { CnsdVccsFreqMeterRecordDetail, CnsdVccsFreqMeterRecordSummary } from '@/types/cnsdVccsFreq';
...
const getSignerStatus = (
  record: CnsdVccsFreqMeterRecordSummary,
...
export const CnsdVccsFreqMeterListPage: React.FC = () => {
  ...
  const [records, setRecords] = useState<CnsdVccsFreqMeterRecordSummary[]>([]);
  ...
  useEffect(() => {
    cnsdVccsFreqMeterService.getYears().then(setAvailableYears).catch(() => setAvailableYears([new Date().getFullYear()]));
  }, []);
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const response = await cnsdVccsFreqMeterService.listRecords(params);
      ...
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        setErrorMessage('Gagal memuat daftar Meter Reading VCCS Frequentis.');
      }
      setRecords([]);
      setTotalCount(0);
    } finally {
  ...
  const handleCreate = async () => {
    ...
    setErrorMessage(null);
    try {
      const record: CnsdVccsFreqMeterRecordDetail = await cnsdVccsFreqMeterService.createRecord({
        date: getCurrentShiftDate(),
        shift_type: getCurrentShiftType(),
      });
      navigate(`/cnsd/vccs-freq-meter/${record.id}`);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response) {
        ...
        if (err.response.status === 409 && data.errors?.existing_record) {
          navigate(`/cnsd/vccs-freq-meter/${data.errors.existing_record.id}`);
          return;
        }
        if (err.response.status === 422) {
          setErrorMessage(data.message ?? 'Tidak ada teknisi CNSD yang bertugas pada shift ini.');
          return;
        }
        setErrorMessage(data.message ?? 'Gagal membuat Meter Reading VCCS Frequentis.');
        return;
      }
      setErrorMessage('Gagal membuat Meter Reading VCCS Frequentis. Coba lagi.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Hapus form Meter Reading VCCS Frequentis ini?')) return;
    try { await cnsdVccsFreqMeterService.deleteRecord(id); void fetchRecords(); } catch { alert('Gagal menghapus form.'); }
  };
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Meter Reading VCCS (Frequentis)</span>
      </div>
      ...
      <PageHeader
        ...
        title="Meter Reading VCCS (Frequentis)"
        subtitle="CNSD Equipment Readiness — VCCS Frequentis"
        actions={canCreate && (
          <Button onClick={handleCreate} className="gap-2 shrink-0" isLoading={isCreating}>
            <Plus size={16} /> Buat Meter Reading
          </Button>
        )}
      />
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 space-y-3">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Cari nomor form, manager, supervisor..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 h-10 rounded-lg border border-gray-300 bg-white text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-transparent" aria-label="Cari Meter Reading VCCS Frequentis" />
          </div>
          {hasActiveFilters && (
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-3 px-4 text-center">
            <div className="h-14 w-14 rounded-full bg-sky-50 flex items-center justify-center"><VccsFreqIcon size={24} className="text-maintenance-cnsd/70" /></div>
            <p className="text-base font-semibold text-slate-700">Belum ada Meter Reading VCCS Frequentis</p>
            <p className="text-sm text-slate-400 max-w-md">Klik <strong>Buat Meter Reading</strong> untuk membuat form VCCS Frequentis baru untuk shift hari ini.</p>
            ...
            <table className="w-full text-sm min-w-[800px]">
              ...
              <tbody>
                {records.map((r) => {
                  ...
                  return (
                    <tr key={r.id} onClick={() => navigate(`/cnsd/vccs-freq-meter/${r.id}`)} tabIndex={0} role="button" className="border-b border-gray-50 hover:bg-gray-50/50 cursor-pointer transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-primary">
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => navigate(`/cnsd/vccs-freq-meter/${r.id}`)} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Detail / Edit"><Edit2 size={16} /></button>
                          <button onClick={() => navigate(`/cnsd/vccs-freq-meter/${r.id}/print`)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Print PDF"><Printer size={16} /></button>
