...
interface PrintSectionBlockProps {
  meta: CnsdAsmgcsMeterSectionMeta;
...
/**
 * CNSD ASMGCS (SAAB) Meter Reading Print View — A4 portrait, B&W.
 * Section A. PERALATAN umbrella + 2 sub-sections (FRONT PANEL / CENTRAL SERVER)
 * + Section B. LINGKUNGAN KERJA.
 */
export const CnsdAsmgcsMeterPrintView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRecord = async () => {
      try {
        const data = await cnsdAsmgcsMeterService.getRecord(Number(id));
        setRecord(data);
      } catch {
        setRecord(null);
      } finally {
  ...
  if (!record) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4">
        ...
        <Button variant="outline" onClick={() => navigate('/cnsd/asmgcs-meter')}>Kembali</Button>
      </div>
    );
  }
  ...
  // Section A umbrella covers FRONT PANEL (1) + CENTRAL SERVER (2);
  // Section B is LINGKUNGAN KERJA (code '4' kept to mirror VCCS Freq partition).
  const peralatanSections = record.sections_meta.filter((s) => s.code === '1' || s.code === '2');
  ...
  return (
    <div className="min-h-screen w-full bg-slate-100 p-4 text-black print:bg-white print:p-0">
      ...
      <div className="print-hide mx-auto mb-4 flex max-w-[210mm] items-center justify-between">
        <Button variant="outline" className="gap-2" onClick={() => navigate(`/cnsd/asmgcs-meter/${record.id}`)}>
