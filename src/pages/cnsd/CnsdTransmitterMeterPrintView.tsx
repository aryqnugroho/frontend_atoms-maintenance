...
const PrintSectionBlock: React.FC<PrintSectionBlockProps> = ({ meta, items }) => {
  ...
  // Transmitter / TX Radio section
  const renderTxGroup = (group: { number: number | null; name: string | null; items: CnsdTransmitterMeterItem[] }, gIdx: number) => {
    ...
    group.items.forEach((item) => {
      ...
      if (isBackup) {
        ...
        rows.push(
          <tr key={item.id} className="text-[10px]">
            <td className="text-center">&nbsp;</td>
            <td className="pl-1">{text(item.tx_label)}</td>
            <td>&nbsp;</td>
  ...
  return <>{groups.flatMap((group, gIdx) => renderTxGroup(group, gIdx))}</>;
};

/**
 * CNSD Transmitter Meter Reading Print View — A4 portrait, B&W.
 *
 * Mirrors paper form 005_Transmitter (FORM C-1):
 *   - Kop band: AirNav logo + "FAS CNS & OTOMASI" (left), Perum LPPNPI (right)
 *   - Title METER READING centered, TRANSMITTER gray bar middle
 *   - Metadata band: LOKASI/TANGGAL (left) · TRANSMITTER (center) · FORM C-1 (right)
 *   - Section 1 items table: 7 cols (NO | FREQUENCY | MERK | STATUS | POWER O/P | MODULASI | KETERANGAN)
 *     with frequency sub-banners and TX 1/TX 2 rows. Back Up Radio status blocked.
 *   - Section 2 (LINGKUNGAN KERJA): 5-col equivalent within 7-col grid via colspan.
 *   - Footer 3-col: TEKNISI | SUPERVISOR | MANAGER TEKNIK
 */
